$( document ).ready(function() {
    ...
    $('body').on('change','#search_date_type_no',function(){
        var value                = $(this).val();
        do_norevenue_search(value)
    });
...
function do_norevenue_search(value) {
    var s_date_start_to      = $('#s_date_start_to');
    var s_date_start_from    = $('#s_date_start_from');
    var s_date_end_to        = $('#s_date_end_to');
    var s_date_end_from      = $('#s_date_end_from');
...
function update_personal_task(task, type, id) {
    ...
    $.ajax({
        ...
        success: function(html){
            ...
            var frame_array = ['customer_list', 'xem_list', 'implement_list','join_list'];
            $.each(frame_array, function( index, value ) {
                css_form(value);
                press(value);
            });
...
function save_personal_task(){
    reset_error();
    var checkOptions = {
        url : BASE_URL+'tasks/edit_personal',
        dataType: "json",
        success: save_personal_task_dt
    };
    $("#task_form").ajaxSubmit(checkOptions);
    return false;
}
...
function save_personal_task_dt(data){
       if(data.flag == 'false') {
         toastr.error('Cập nhật không thành công!', 'Thông báo');
...
function add_norevenue_task() {
    reset_error();
    var checkOptions = {
        url : BASE_URL+'tasks/add_personal',
        dataType: "json",
        success: add_norevenue_task_data
    };
    $("#task_form").ajaxSubmit(checkOptions);
    return false;
}
...
function add_norevenue_task_data(data) {
    if(data.flag == 'false') {
        $.each(data.errors, function( index, value ) {
            element = $( '#my_modal span[for="'+index+'"]' );
            element.prev().addClass('has-error');
...
function delete_norevenue(id) {
    var flag       = true;
    var ids        = new Array();
    var table      = $('[data-table="norevenue"]');
    var manage_row = $('.manage-row-options[data-table="norevenue"]');
    var check_all  = table.find('.check_tatca');
    check_all      = check_all.closest('label').prev();

    if (typeof id == 'undefined') {
        var checkbox = $("[data-table='norevenue'] .file_checkbox:checked");
        if(checkbox.length) {
            $(checkbox).each(function( index ) {
                ids[ids.length] = $(this).val();
            });
