$( document ).ready(function() {
    // autocomplete
    var frame_array = ['customer_list', 'xem_list','join_list','implement_list', 'trangthai_list'];
    ...
    date_time_picker_field_report($('#adv_date_start_from'), DATE_FORMAT);
    date_time_picker_field_report($('#adv_date_start_to'), DATE_FORMAT);
    date_time_picker_field_report($('#adv_date_end_from'), DATE_FORMAT);
    date_time_picker_field_report($('#adv_date_end_to'), DATE_FORMAT);
   ...
   $('body').on('click','.my-table tbody tr td.cb',function(){
    ...
    $('body').on('click','#btn_search_advance',function(){
        ...
        $('body').on('click','#btn_search_advance_task_list',function(){
        if($('#task_section').is(":visible")) {
            set_hidden_input();
    ...
    // });
    $('body').on('click','#search_press',function(){
...
// alert('chan doi');
        ...
        switch(value) {
            case 'today':
                var current_date = get_current_date();
                s_date_start_to.val(current_date + ' 23:59:59');
                s_date_start_from.val(current_date + ' 00:00:00');
            ...
            case 'weekend':
                ...
                s_date_start_to.val(lastDay + ' 23:59:59');
                s_date_start_from.val(firstDay + ' 00:00:00');
                // s_trangthai.val('0,1');
...
function do_quick_search(data_parent) {
    ...
    var s_join_html     = element_parent.find('.s_join_html');
...
function set_project_hidden_input() {
    ...
    var span_join_item = $('#join_list .item');
    if(span_join_item.length) {
        $( span_join_item ).each(function() {
            var span_element  = $(this);
            item[item.length] = span_element.find('.join').val();
            item_html[item_html.length] = span_element[0].outerHTML;
        });
    }
    ...
    item_string      = item.join();
    item_html_string = item_html.join('');
    s_join.val(item_string);
    s_join_html.html(item_html_string);
...
function startSearch (project_id) {
    ...
    var s_join_html      = tr_element.find('.s_join_html');
...
function set_project_form_input() {
    ...
    // html = s_join_html.html();
    $(html).insertBefore( "#join_result" );
...
function add_project() {
	...
	$.ajax({
		...
		success: function(html){
		   ...
		   var frame_array = ['customer_list', 'xem_list', 'implement_list', 'join_list', 'create_task_list', 'pheduyet_task_list', 'progress_list'];
		   $.each(frame_array, function( index, value ) {
			  css_form(value);
			  press(value);
		   });
