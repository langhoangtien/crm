<?php 
class BizTaskProgress extends CI_Model{
	...
	public function countItem($arrParam = null, $options = null){	
		$taskTable = $this->model_load_model('Task');
	...
	public function saveItem($arrParam = null, $options = null) {
		if($options['task'] == 'add') {
			...
			$sqlString   = 'UPDATE ' .$this->db->dbprefix($this->_table)
						  . ' SET progress = -1'
						  . ' WHERE task_id IN ('.$task_ids.') AND pheduyet = -1';
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'public-list'){
			...
			$this->db->select("DATE_FORMAT(p.date_pheduyet, '%d-%m-%Y') as created", FALSE);
			...
			if(!empty($result)) {
				foreach($result as &$val) {
					...
					elseif($val['pheduyet'] == 1 || $val['pheduyet'] == 2)
	...
	function do_progress($level, $options, $key = null) {
		...
		#Sai ở đây , cập nhật tất cả lv3 cho lv2 ??????
		// echo "<pre>";
		// var_dump($level);die();
		if(!empty($level)) {
		 ...
		 // var_dump($last_level);die();
			$parent_id = $options['task_info']['parent'];
			if($parent_id > 0) {
				$new_parent_progress = 0;
				foreach($last_level as $task) {
					if($task['parent'] == $options['task_info']['parent']){
						$new_parent_progress = $new_parent_progress + $task['percent'] * 0.01 * $task['progress'];
					}
					
				}

                if($new_parent_progress == 100){
                    $new_date_finish = @date("Y-m-d H:i:s");
                    $new_trangthai   = 2;
                }else{
                    if($new_parent_progress == 0)
                        $new_trangthai   = 0;
                    else
                        $new_trangthai   = 1;

                    $new_date_finish = '0000/00/00 00:00:00';
                }

				//update parent
				$taskTable->saveItem(array('id'=>$parent_id, 'progress'=>$new_parent_progress, 'trangthai'=>$new_trangthai, 'date_finish'=>$new_date_finish), array('task'=>'update-progress'));
					...
					foreach($level as &$l) {
						foreach($l as &$task) {
							if($parent_id == $task['id']) {
                                ...
                                $task['trangthai']   = $new_trangthai;
                                $task['date_finish'] = $new_date_finish;
								$parent_item = $task;
				...
				$id = $options['task_info']['parent'];
				$parent = $this->Task->get_task_item($id)['parent'];
				$options['task_info'] =array('id'=>$id,'parent'=>$parent);
	...
	public function solve($arrParam = null, $options = null) {
		...
		$option['task_info'] = array('id'=>$arrParam['id'],'parent'=>$arrParam['parent'],'project_id'=>$arrParam['project_id']);
		$taskTable 		= $this->model_load_model('Task');
		$task 			= $arrParam;
		if($options['task'] == 'edit' || $options['task'] == 'remove'){
			#Lấy tất cả các công việc theo projectid
		...
		$this->do_progress($level, $option, $arrParam['key']);
	...
	function handling($arrParam = null, $options = null) { 
		...
		if($progress_item['progress'] == -1) { // chỉ cập nhật trạng thái
			...
			// up progress = -1 => progress hiện tại
