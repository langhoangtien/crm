    ...
    <thead>
      <tr>
        ...
        <th><?php echo lang('common_name'); ?></th>
		  <?php if($add){ ?>
        <th style="text-align: center;""><?php echo 'Đã xuất/Tổng số lượng'; ?></th>
		  <?php } ?>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td id="so-luong" class="text-center">
                <?php 
                echo round($item->qtyStockOut,2) . '/' . round($item->qtyOrigin,2); 
                ?>
        </td>
			<?php } ?>
        <td class="text-center">
        	...
        	<?php if (!empty($mode) && $mode == 'by_sale') { ?>
        		<a href="#" class="so-luong-xuat-kho xeditable" data-type="text" data-value-origin="<?php echo round($item->qtyOrigin,2); ?>" data-number-type="unsigned" data-validate-number="true"  data-pk="1" data-name="quantity" data-url="<?php echo site_url('stock_out/edit_item/' . $item->item_id); ?>" data-title="<?php echo lang('common_quantity') ?>"><?php echo to_quantity($item->totalQty); ?></a>
        	<?php } else {?>
        	<a href="#" class="so-luong-xuat-kho xeditable" data-type="text" data-number-type="unsigned" data-validate-number="true"  data-pk="1" data-name="quantity" data-url="<?php echo site_url('stock_out/edit_item/' . $item->item_id); ?>" data-title="<?php echo lang('common_quantity') ?>"><?php echo to_quantity($item->totalQty); ?></a>
        	<?php } ?>
        </td>
        <td class="text-center">
      ...
      <?php } ?>
      <?php } else {?>
		  <tr style="cursor: pointer;"><td colspan="5"><div class="col-log-12" style="text-align: center; color: darkred;">Bạn đã xuất đủ số lượng, quay lại đơn hàng tạm dừng để hoàn thành đơn <a href="<?php echo base_url().'sales/unsuspend/'.$sale_id; ?>">click here</a></div></td></tr>
	  <?php } ?>
 ...
 $(document).ready(function(){
		$('.xeditable').editable({
	    	...
	    	validate: function(value) {	
                ...
                <?php if (!empty($mode) && $mode == 'by_sale') { ?>
    	             if($(this).data('name') == 'quantity' && value > tong_so_luong) {
                        return <?php echo json_encode('Số lượng không được vượt quá số lượng cần xuất, số lượng cần xuất ra là '); ?>+tong_so_luong;
                    }
                <?php } ?>
	    	...
	    	success: function(response, newValue) {
                ...
                $(this).parent().prev().find('.so-luong-xuat-kho').text(response.tong_so_luong);
                // Thay đổi lại số lượng so sánh
                var vitri = $(this).parent().prev().find('.so-luong-xuat-kho');
