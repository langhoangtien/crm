<table class="tablesorter table table-hover data-n9-table" data-table="contract_sales" style="border-top: 0;">
    ...
    if(!empty($cart)) {
        foreach($cart as $item) {
            ...
            <tr style="cursor: pointer;">
                ...
                <td class="cb center"><?php echo number_format($quantity,'2','.',','); ?></td>
    ...
    $total_all = to_currency(abs($total_all));
    ...
    if(count($discount_items)>0) {
        ...
        foreach($discount_items as $item)
            $discount_total = $discount_total + abs($item['price']*$item['quantity']);

        $info[] = 'Giảm giá: ' . '<span class="bold">'.to_currency(abs($discount_total)).'</span>';
    }
