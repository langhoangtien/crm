...
function n9_grid_callback(data_table, result) {
	if(data_table == 'receiving_store_payment_modal') {
		$('#total_conlai').html(result.con_lai);
	}
}

var receiving_mode = '<?php echo $receiving_mode; ?>';

$(document).ready(function(){
	if(receiving_mode == 'store_account_payment')
		var url_modal = BASE_URL + 'receivings/modal_store_payment';
	else if(receiving_mode == 'vat_order')
		var url_modal = BASE_URL + 'receivings/modal_vat_order';

    $('body').on('click','#cong_no_btn',function(){
    $.ajax({
        type: "POST",
        url: url_modal,
        data: {
        ...
        success: function(html){
        	if(html == 'no-receiving')
        		toastr.error('Bạn đang ở chế độ nhập hàng khác', 'Lỗi');
        	else if(html == 'no-supplier')
        		toastr.error('Chưa chọn nhà cung cấp', 'Lỗi');
        	else {
...
show_feedback('success', 'Tạo nhu cầu bên thứ 3 thành công');
...
<?php
if(isset($_SESSION['receiving_store_payment_modal_filter']))
	unset($_SESSION['receiving_store_payment_modal_filter']);
?>
