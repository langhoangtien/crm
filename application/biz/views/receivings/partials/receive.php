...
<div id="pdf_content">
	...
	<div id="pdf_customer">
		...
		<?php if ($this->config->item('show_comment_on_receipt') == 1) :?>
		<p> Ghi chú: <?php echo nl2br($comment); ?></p>
		<?php endif;?>
		<p>Tổng nợ cũ: <span><?php if (isset($tong_no_dau)) echo NumberFormatToCurrency($tong_no_dau); ?></span></p>
	</div>
	<div class="w100 clb table-responsive">
		<table id="pdf_tbl_items" class="w100 table">
			<tbody>
				...
				<?php foreach($cart as $line=>$item) { ?>
					...
					{
						$total_discount += (abs ( $item ['price'] ) * abs ( $item ['quantity'] ) - abs ( $item ['price'] ) * abs ( $item ['quantity'] ) * $item ['discount'] / 100);
					}
        ...
        <tr>
				...
				foreach ( $taxes as $name => $value ) {
							?>
				<tr>
					<td colspan="8" class="border-bottom border-top text-bold"><?php echo $name.': ';  echo NumberFormatToCurrency(abs($value)); ?>
					</td>
				</tr>
							<?php } ?>
					<?php } ?>
				...
				<?php if(!empty($payments)) {?>
					...
					<tr>
						<td colspan="8" class="border-bottom border-top text-bold"><?php echo $payment['payment_type'] . ': ' . NumberFormatToCurrency(abs($payment['amount'])); ?></td>
					</tr>
				...
				<tr>
					<td colspan="8" class="border-bottom border-top text-bold"><?php echo 'Tổng nợ cuối'.': ';
					if (isset($tong_no_cuoi)) { echo NumberFormatToCurrency($tong_no_cuoi); } ?>
					</td>
				</tr>
                ...
                <tr>
					<td colspan="9" style="border-left: none; border-right: none; border-bottom: none;"></td>
				</tr>
			</tbody>
		</table>
	</div>
