...
<?php $sms_templates_ = json_encode((!empty($sms_templates))?$sms_templates:'');?>
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
	...
	<div class="form-group">
		...
		<div class =" form-group col-xs-12 col-sm-12 col-md-6 col-lg-6">
			...
			<select id="selectedTemp" class="form-control">
				<?php foreach($sms_templates as $sms_template):?>
					<option value="<?php echo $sms_template['id'];?>" <?php echo (!empty($sms_campain['sms_id'])&&$sms_campain['id']==$sms_template['id'])? 'selected': '';?>><?php echo $sms_template['title'];?></option>
				<?php endforeach;?>
			</select>
		</div>
		
		
		<?php echo form_label('Nội dung SMS :', 'content',array('class'=>' col-sm-3 col-md-3 col-lg-2 ')); ?>
		...
		<div class="form-group col-sm-12 col-md-12 col-lg-12">
			<?php echo form_textarea(array(
																			...
																			'value'		=> (!empty($sms_campain['sms_id']) && !empty($sms_templates))? $sms_templates[$sms_campain['sms_id']]['message']:array_values($sms_templates)[0]['message'],
															));?>
												 <?php echo display_ckeditor($ckeditor);?>
												
				<span for="sms_content" class="text-danger errors"></span>
			</div>
		</div>
	</div>
	<div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12">
	<label><?php echo (empty($sms_campain['sms_id']))? 'Cài đặt thời gian chiến dịch:': 'Chỉnh sửa thời gian chiến dịch:';?></label>
		...
		<?php if(($sms_campain['send_minutes'] != '*' && $sms_campain['send_hours'] != '*' && $sms_campain['send_day_of_month'] != '*' && $sms_campain['send_month'] != '*')&& $sms_campain['iterative_time'] == 0 ):?>
		<?php $schedule = 'tab_schedule0'?>
		<?php elseif($sms_campain['send_minutes'] != '*' && $sms_campain['send_hours'] == '*' && $sms_campain['send_day_of_month'] == '*' && $sms_campain['send_month'] == '*'):?>
...
<script type="text/javascript">
	...
	$('#chk_edit_enable').on('click',function(){
		...
		var set_minutes         = '<?php echo empty($sms_campain['send_minutes'])? 0:$sms_campain['send_minutes'];?>';
		var set_hours           = '<?php echo empty($sms_campain['send_hours'])? 0:$sms_campain['send_hours'];?>';
		var set_dayOfWeek       = '<?php echo empty($sms_campain['send_day_of_week'])? 0:$sms_campain['send_day_of_week'];?>';
		var set_months          = '<?php echo empty($sms_campain['send_month'])? 0:$sms_campain['send_month'];?>';
		var set_day             = '<?php echo empty($sms_campain['send_day_of_month'])? 0:$sms_campain['send_day_of_month'];?>';
		...
		var redirect = 	'<?php echo site_url("customers/manage_sms_campain/");?>';
		$('#save').click(function(){
