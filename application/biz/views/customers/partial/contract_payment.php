...
?>
    <div class="clearfix top-control">
        ...
        <div class="col-xs-6 pull-right">
            <input  type="button" class="btn btn-red btn-lg btn-right" data-table="contract_payment" data-table-reload="contract_delivery" data-url="<?php echo base_url() . 'customers/contract_payment_delete' ?>" value="Xóa"/>
        </div>
        <input type="hidden" name="s_contract_id" class="data-n9-s" data-table="contract_payment" value="<?php echo $contract_id; ?>" />
    </div>
    <table class="tablesorter table table-hover data-n9-table" data-table="contract_payment" data-url="<?php echo base_url() . 'sales/contract_payment_store/' ?>" data-currentPage="1">
        <thead>
        <tr>
            <th class="leftmost" style="width: 20px;">
                <input type="checkbox"><label for="select_all" class="check_tatca"><span></span></label>
            ...
            <th data-field="name"<?php if($field_sort == 'name') echo ' class="header '.$class_sort.'"'; ?>>Giai đoạn</th>
            <th style="width: 15%;" data-field="date_payment"<?php if($field_sort == 'date_payment') echo ' class="header '.$class_sort.'"'; ?>>Thời gian giao hàng</th>
            <th style="width: 20%;">Số tiền</th>
    <?php if($type != 'rule'):   ?>
