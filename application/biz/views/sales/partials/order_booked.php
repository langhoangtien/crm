...
<?php $configRemHaMy = $this->config->item('company_user') == 'remHaMy'; ?>
<div id="pdf_content">
	<div id="pdf_header">
    ...
    <?php if (!$configRemHaMy):?>
		<table id="pdf_tbl_items" class="w100 table">
			<tbody>
				...
				{
				if($item['name'] != lang('common_discount') && $item['quantity'] != 0 )
					{
            $total_money +=($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100);
				...
				?>
         <?php if($item['name'] != lang('common_discount') && $item['quantity'] != 0 ):?>
					<tr>
						<td><?php echo $stt; ?></td>
          ...
          <tr>
            <td class="border-bottom text-bold" colspan="9"><?php echo lang('common_total_money').': '; echo NumberFormatToCurrency(abs($total_money)); ?></td>
					...
					<!-- View discount if it exists: $total_discount is negative number so we have to put a munus before this one-->
					<?php if(-$total_discount>0):?>
					 <tr>
            <td class=" text-bold" colspan="9"><?php echo lang('common_discount').': '; echo NumberFormatToCurrency(abs($total_discount)); ?></td>
          </tr>
				  <?php endif;?>
				...
				<tr>
					<td class="border-bottom border-top text-bold" colspan="9"><?php if (isset($is_integrated_credit_sale)&& $is_integrated_credit_sale || sale_has_partial_credit_card_payment() && ($payment['payment_type'] == lang('common_credit') ||  $payment['payment_type'] == lang('sales_partial_credit'))) { ?>
		...
		<table id="pdf_tbl_items" class="w100 table">
			<tbody>
				...
				{
                    if (isset($changedCart[$item['item_id']])) {
                        if ((isset($cartItemsAttributeSetType[$line]) && $cartItemsAttributeSetType[$line] == 'rem_roman') || 
                            ($cartItemsAttributeSet[$line] == $ATTR_SET_REM_KEO_VAI_RONG_CC_260CM )|| 
                            ($cartItemsAttributeValue[$line][$ATTR_BEO_HOAC_YEM]->entity_value == 'co')) {
                                echo $cartItemsAttributeValue[$line][$ATTR_MET_VAI]->entity_value.',';
                            $changedCart[$item['item_id']]['quantity'] += $cartItemsAttributeValue[$line][$ATTR_MET_VAI]->entity_value;
                        } else {
                            $changedCart[$item['item_id']]['quantity'] += $item['quantity'];
                        }
                    } else {
                        $changedCart[$item['item_id']] = $item;
                        if (isset($cartItemsAttributeSetType[$line]) && $cartItemsAttributeSetType[$line] == 'rem_roman'|| 
                            ($cartItemsAttributeSet[$line] == $ATTR_SET_REM_KEO_VAI_RONG_CC_260CM )|| 
                            ($cartItemsAttributeValue[$line][$ATTR_BEO_HOAC_YEM]->entity_value == 'co')) {
                            $changedCart[$item['item_id']]['quantity'] = $cartItemsAttributeValue[$line][$ATTR_MET_VAI]->entity_value;
                ...
                foreach($changedCart as $item_id => $item)
				{
				?>
		...
		<div id="policy">
                ...
                <p style=" font-weight: bold; text-align: center"><?php echo lang('common_return_policy')?></p>
