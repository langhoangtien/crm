	...
	<div class="w100 clb table-responsive">
		<table id="pdf_tbl_items" class="w100 table">
          ...
          $total_money +=($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100);
					...
					if($item['name']!= lang('common_discount')&&$item['name']!='')
					{
				?>
				...
				<tr>
				<!-- Tổng tiền hàng -->
                    <td class="border-bottom text-bold" colspan="9"><?php echo lang('common_total_money').': '; echo NumberFormatToCurrency(abs($total_money)); ?>
                    </td>
				</tr>
				...
				<?php endif;?>
                    <?php if ($this->config->item('group_all_taxes_on_receipt')) { ?>
						<?php 
						$total_tax = 0;
						foreach($taxes as $name=>$value) { $total_tax+=$value; }
						?>
						<tr>
	                        <td colspan="9"  class="border-bottom border-top text-bold"><?php echo lang('common_tax').': ';  echo NumberFormatToCurrency($total_tax); ?>
	                        </td>
						</tr>
					<?php } else { ?>
						<?php foreach($taxes as $name=>$value) { ?>
						<tr>
	                        <td colspan="9"  class="border-bottom border-top text-bold"><?php echo $name.': ';  echo NumberFormatToCurrency($value); ?>
	                        </td>
						</tr>
						<?php }; ?>
					<?php } ?>
                ...
                <!-- Tổng đơn hàng -->
                    <td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_total').': '; 
                        echo $this->config->item('round_cash_on_sales') && $is_sale_cash_payment ? NumberFormatToCurrency(round_to_nearest_05($total)) : NumberFormatToCurrency($total); ?>
                    </td>
				</tr>

				<!-- Tiền mặt và sổ ghi nợ -->
				<?php foreach($payments as $payment_id => $payment) { ?>
				<tr>
                    <td colspan="9" class="border-bottom border-top text-bold">
                    <?php if (isset($is_integrated_credit_sale) && ($is_integrated_credit_sale || sale_has_partial_credit_card_payment()) && ($payment['payment_type'] == lang('common_credit') ||  $payment['payment_type'] == lang('sales_partial_credit'))) { ?>
					...
					<?php } //end if
						...
						<?php } //end else
                        echo $this->config->item('round_cash_on_sales') && $payment['payment_type'] == lang('common_cash') ?  ': '.  NumberFormatToCurrency(round_to_nearest_05($payment['payment_amount'])) : ': '.  NumberFormatToCurrency($payment['payment_amount']); ?>
                    </td>
				</tr>
					...
					<?php if (strpos($payment['payment_type'], lang('common_giftcard'))!== FALSE) {?>
						...
						<tr>
							<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('sales_giftcard_balance'); 
                                ...
                                echo NumberFormatToCurrency($this->Giftcard->get_giftcard_value(end($giftcard_payment_row))); ?>
                            </td>
						</tr>
					<?php }?>
				<?php }?>
				...
				<?php if ($amount_change >= 0) {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_change_due').': '; 
                            echo $this->config->item('round_cash_on_sales')  && $is_sale_cash_payment ? NumberFormatToCurrency(round_to_nearest_05($amount_change)) : NumberFormatToCurrency($amount_change) ?>
                        </td>
					</tr>
				<?php } else { ?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_amount_due').': '; 
                            echo $this->config->item('round_cash_on_sales')  && $is_sale_cash_payment ?  NumberFormatToCurrency(round_to_nearest_05($amount_change * -1)) : NumberFormatToCurrency($amount_change * -1); ?>
                        </td>
					</tr>
				<?php } ?>

				<!-- Tổng nợ cuối -->
				<?php if (isset($tong_no_cuoi) && $tong_no_cuoi !== FALSE && !$this->config->item('hide_store_account_balance_on_receipt')) {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold">Tổng nợ cuối: 
                            <?php echo NumberFormatToCurrency($tong_no_cuoi); ?>
                        </td>
					</tr>
				<?php } ?>
				...
				<?php if ($this->config->item('enable_customer_loyalty_system') && isset($sales_until_discount) && !$this->config->item('hide_sales_to_discount_on_receipt') && $this->config->item('loyalty_option') == 'simple') {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_sales_until_discount').': '; 
                            echo $sales_until_discount <= 0 ? lang('sales_redeem_discount_for_next_sale') : to_quantity($sales_until_discount); ?>
                        </td>
					</tr>
				<?php } ?>
				<?php if ($this->config->item('enable_customer_loyalty_system') && isset($customer_points) && !$this->config->item('hide_points_on_receipt') && $this->config->item('loyalty_option') == 'advanced') {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_points').': ';
                            echo NumberFormatToCurrency(to_quantity($customer_points)); ?>
                        </td>
					</tr>
				<?php } ?>
