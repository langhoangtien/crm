<?php
$filter   = isset($_SESSION['sales_model_filter'])?$_SESSION['sales_model_filter']:array();
if(isset($filter['current_page']))
    $current_page = $filter['current_page'];
else
    $current_page = 1;

$keywords = isset($filter['keywords'])?$filter['keywords']:array();
if(!isset($filter['col'])) {
    $field_sort = 'sale_id';
        ...
        <div class="modal-body">
            <div class="control row">
                ...
                <div class="col-xs-6 right">
                    <input type="text" class="form-control data-n9-s" data-table="sale_modal" name="s_keywords" id="search" value="<?php if(isset($filter['keywords'])) echo $filter['keywords']; ?>" placeholder="Tìm kiếm mã đơn hàng">
                </div>
            </div>
            <div class="panel-body nopadding table_holder table-responsive">
                <table class="tablesorter table table-hover data-n9-table" data-table="sale_modal" data-scroll="false" data-url="<?php echo base_url() . 'sales/modal_ds_store/'; ?>" data-currentpage="<?php echo $current_page; ?>">
                    <thead>
                    <tr>
