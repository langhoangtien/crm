...
<div class="manage_buttons hidden-print">
		...
		<div class="col-md-6"><?php
			...
			<div class="hidden-print search no-left-border">
				<ul class="list-inline print-buttons">
                        ...
                        <?php if( $sale_mode != 'return') : ?>
                        <li style="padding-right: 0px;">
							<?php
							 if ($sale_id_raw != lang('sales_test_mode_transaction') && !$store_account_payment && $this->Employee->has_module_action_permission('sales', 'edit_sale', $this->Employee->get_logged_in_employee_info()->person_id) && $sale_mode != 'vat_order'){
								...
								<button class="btn btn-primary btn-lg hidden-print" id="edit_sale"> <?php echo lang('receivings_service_edit'); ?> </button>
    ...
    <?php
    if(!empty($sale_document) && $sale_mode == 'sale') {
        foreach($sale_document as $key => $doucment) {
    ?>
            <li style="padding-right: 0px;">
                <button class="btn btn-primary btn-lg hidden-print" id="print_button" onclick="print_document(<?php echo $key; ?>);"> <?php echo $doucment['title_quotes_contract']; ?> </button>
            </li>
    <?php
        }
    }
    ...
    ?>
					...
					<li>
						<button class="btn btn-primary btn-lg hidden-print" id="fufillment_sheet_button" onclick="window.open('<?php echo site_url("sales/receipt/$sale_id_raw"); ?>', 'blank');" > <?php echo lang('sales_receipt'); ?></button>
                                    ...
                                    <?php }?>
                                    <?php
                                            if($this->config->item('config_sales_receipt_pdf_size')=='a8' && $this->input->get('fulfillment')==NULL ){ if(empty($type)) $type ='';?>
                                                <li style="padding-right: 0px;">
			...
			<div class="buttons-list">
				<div class="pull-right-btn">
                    ...
                    <?php if($this->config->item('company_user') == 'remHaMy'):?>
                        <?php if($suspend == 2):?>
                                <li>
                                    <a  class ="btn btn-primary" href = "ExportExcel/orderShowPriceExcel/<?php echo $sale_id_raw.'/'.$type_receipt?>" target = "_blank">Xuất file excel</a>
                                </li>
                        <?php elseif ($suspend == 1):?>
                                <li>
                                    <a class ="btn btn-primary" href = "ExportExcel/orderBookExportExcel/<?php echo $sale_id_raw.'/'.$type_receipt?>" target = "_blank">Xuất file excel</a>
                                </li>                        
                        <?php endif;?>
                    <?php endif;?>
						<li>
                        ...
                        <li>
                            <a href="<?php echo base_url() . 'sales/suspended'; ?>" class="btn btn-primary btn-lg hidden-print" ><?php echo lang('common_back');?></a>
                        </li>
						<li>
							<button class="btn btn-primary btn-lg hidden-print" id="new_sale_button_1" onclick="don_hang_moi()" > <?php echo lang('sales_create_request_customer'); ?> </button>
						</li>
					</ul>
				</div>
			</div>
...
<div class="row manage-table receipt_<?php echo $this->config->item('receipt_text_size') ? $this->config->item('receipt_text_size') : 'small';?>" id="receipt_wrapper">
	<div class="col-md-12" id="receipt_wrapper_inner">
		<div class="panel panel-piluku">
            <div id="pdf_section">
                <iframe id="sale_section" src="<?php echo base_url(); ?>sales/sale_section/<?php echo $sale_id_raw.'/'.$type_receipt; ?>?template=<?php echo $template; ?>" width="100%" height="100%" style="border: none;">
...
function print_receipt()
 {
    ...
    var html = $('#sale_section').contents().find("html").html();
...
<script type="text/javascript">
	...
	function don_hang_moi() {
		coreAjax.call(
			'<?php echo site_url("sales/cancel_sale");?>',
		);
		window.location='<?php echo site_url('sales'); ?>'

	}

    function print_document(key) {
       var string = $('#print_document_'+key).html();
       newWin = window.open("");
...
<?php
    if(!empty($sale_document) && $sale_mode == 'sale') {
        foreach($sale_document as $key => $document) {
?>
    <div id="print_document_<?php echo $key; ?>" style="display: none;">
        <?php echo $document['content_quotes_contract']; ?>
    </div>
<?php
        }
    }
