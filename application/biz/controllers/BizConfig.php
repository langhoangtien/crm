...
{
	protected $ecommerce_operations = [
	...
	function save()
	{
        ...
        if(!empty($post['config_adjusted_cost_price'])) {
            $config_adjusted_cost_price = serialize($post['config_adjusted_cost_price']);
        }else
            $config_adjusted_cost_price = '';
		...
		$batch_save_data=array(
			...
			'default_receiving_VAT'=>$this->input->post('default_receiving_VAT')?$this->input->post('default_receiving_VAT'):'',	
      		'default_receiving_VAT_name'=>$this->input->post('default_receiving_VAT_name')?$this->input->post('default_receiving_VAT_name'):'',					
			'default_tax_1_rate'=>$this->input->post('default_tax_1_rate')?$this->input->post('default_tax_1_rate'):'',		
			'default_tax_1_name'=>$this->input->post('default_tax_1_name')?$this->input->post('default_tax_1_name'):'',	
			'default_tax_2_rate'=>$this->input->post('default_tax_2_rate')?$this->input->post('default_tax_2_rate'):'',
			'default_tax_2_name'=>$this->input->post('default_tax_2_name')?$this->input->post('default_tax_2_name'):'',
			'default_tax_2_cumulative' => $this->input->post('default_tax_2_cumulative') ? 1 : 0,
			'default_tax_3_rate'=>$this->input->post('default_tax_3_rate')?$this->input->post('default_tax_3_rate'):'',
			'default_tax_3_name'=>$this->input->post('default_tax_3_name')?$this->input->post('default_tax_3_name'):'',
			'default_tax_4_rate'=>$this->input->post('default_tax_4_rate')?$this->input->post('default_tax_4_rate'):'',
			'default_tax_4_name'=>$this->input->post('default_tax_4_name')?$this->input->post('default_tax_4_name'):'',
			'default_tax_5_rate'=>$this->input->post('default_tax_5_rate')?$this->input->post('default_tax_5_rate'):'',
			'default_tax_5_name'=>$this->input->post('default_tax_5_name')?$this->input->post('default_tax_5_name'):'',
			...
			'sales_add_more_cus_for_visa'  => $this->input->post('config_sales_add_more_cus_for_visa') ? 1 : 0,
            ...
            'shift_category_id'=>$this->input->post('shift_category_id'),
            'shift_user_id' => $this->input->post('shift_user_id'),
			...
			'number_of_decimals' => $this->input->post('number_of_decimals')?$this->input->post('number_of_decimals'):0,
			...
			'store_account_statement_message' => $this->input->post('store_account_statement_message')?$this->input->post('store_account_statement_message'):'',
			...
			'config_template_sale' 		    => $this->input->post('config_template_sale'),
            'config_template_order_sale'    => $this->input->post('config_template_order_sale'),
            ...
            'quote_constract_type_default' => 'a:10:{i:0;s:4:"BMHD";i:1;s:4:"BMBG";i:2;s:4:"BMNT";i:3;s:4:"BMTT";i:4;s:6:"BMNTTL";i:5;s:6:"BMYCTT";i:6;s:4:"BMDH";i:7;s:4:"BMBH";i:8;s:6:"BMBBBG";i:9;s:5:"BMHĐ";}',
            'import_quantity' => $this->input->post('import_quantity'),
            'config_price_imported' => $this->input->post('config_price_imported'),
            'sale_order_has_service' => $this->input->post('sale_order_has_service') ? 1 : 0,
            'commission_method' => $this->input->post('commission_method'),
            'commission_time_method' => $this->input->post('commission_time_method'),
            'min_profit' => (float)$this->input->post('min_profit'),
            'min_profit_commission' => (float)$this->input->post('min_profit_commission'),
            'customer_balance'   =>   $this->input->post('customer_balance'),
            'customer_balance_2' =>   $this->input->post('customer_balance_2'),
            'supplier_balance'   =>   $this->input->post('supplier_balance'),
            'supplier_balance_2' =>   $this->input->post('supplier_balance_2'),
            'config_adjusted_cost_price' => $config_adjusted_cost_price,
            //admin section
            // 'config_vat_order'   =>   $this->input->post('config_vat_order'),
            'config_default_sale_quantity'   =>   $this->input->post('config_default_sale_quantity'),
			'enable_scope_of_view_location' => $this->input->post('config_default_sale_quantity')?1:0,
			'ma_khach_hang_prefix' => $this->input->post('ma_khach_hang_prefix') ? $this->input->post('ma_khach_hang_prefix'):$this->config->item('ma_khach_hang_prefix'),
			'ma_khach_hang_bat_dau_tu' => $this->input->post('ma_khach_hang_bat_dau_tu') ? $this->input->post('ma_khach_hang_bat_dau_tu'):$this->config->item('ma_khach_hang_bat_dau_tu'),
			'enable_customer_dashboard' => $this->input->post('enable_customer_dashboard') ? 1 : 0,

		);
		
		$batch_save_data['ecommerce_platform'] = $this->input->post('ecommerce_platform') ? $this->input->post('ecommerce_platform') : '';
		$batch_save_data['ecom_store_location'] = $this->input->post('ecom_store_location') ? $this->input->post('ecom_store_location') : '';

		if ( $batch_save_data['ecommerce_platform'] == 'woocommerce') {
			$operations = [];
			foreach ($this->ecommerce_operations as $operation) {
				if (!empty($this->input->post($operation))) {
					$operations[] = $operation;
				}
			}
			$batch_save_data['ecommerce_cron_sync_operations'] = json_encode($operations);

			// $batch_save_data['ecommerce_platform'] = $this->input->post('ecommerce_platform') ? $this->input->post('ecommerce_platform') : 0;
			// $batch_save_data['ecom_store_location'] = $this->input->post('ecom_store_location') ? $this->input->post('ecom_store_location') : 0;

			$batch_save_data['woo_version'] = $this->input->post('woo_version')? $this->input->post('woo_version') : '';
			$batch_save_data['woo_api_url'] = $this->input->post('woo_api_url')? $this->input->post('woo_api_url') : '';
			$batch_save_data['woo_api_key'] = $this->input->post('woo_api_key')? $this->input->post('woo_api_key') : '';
			$batch_save_data['woo_api_secret'] = $this->input->post('woo_api_secret')? $this->input->post('woo_api_secret') : '';
		}

		// if($this->input->post('ma_khach_hang_prefix') && $this->input->post('ma_khach_hang_prefix') != $this->config->item('ma_khach_hang_prefix')){
		...
		// var_dump($batch_save_data['return_policy']);die;
    ...
    function validate_child_category() {
        $post = $this->input->post();
        if(!empty($post)) {
            $this->db -> select('COUNT(id) AS totalItem')
                      -> from('categories AS c')
                      -> where('c.deleted',0)
                      -> where('c.parent_id', $post['category_id']);
