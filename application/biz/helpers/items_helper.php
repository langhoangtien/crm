...
function thay_doi_key_theo_line($data){
	$cart = array();
	if(!empty($data)) {
	    foreach($data as $val) {
	        $cart[$val['line']] = $val;
...
function getQtyOfItemByMeasure($itemId, $qty, $measureId) {
	if (empty($measureId)) {
		return $qty;
	}
	
	$CI =& get_instance();
	$itemMeasure = $CI->ItemMeasures->getMeasuresByIdAndItemId($itemId, $measureId);
	
	$convertedQty = $qty;
	if ($itemMeasure) {
		$convertedQty = $qty * $itemMeasure->qty_converted;
	}
	return $convertedQty;
}
...
function getItemConvertedQtyAllLocation($itemId) {
	...
	# lấy số lượng tồn trong các kho
	$allQty = $CI->Item_location->getQtyOfEachLocation($itemId);
	# Chắc là lấy đơn vị tính chứ gì 
...
function qtyToString($itemId, $qty) {
	...
	$convertedQtyToString = to_quantity($qty) . ' ' . $measure_name. ' ' . '(';
	
	$convertedQty = getItemConvertedQty($qty, $itemInfo->measure_id, $measures);
...
function get_items_barcode_data($item_ids)
{
	...
	foreach ($item_ids as $item_id)
	{
		...
		$item_category_name =$item_info->category_name;
		...
		if($CI->config->item('barcode_price_include_tax'))
		{
			...
			{
				$result[] = array('category_name'=>	$item_category_name,'name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-size:12px;">'.to_currency($regular_item_price).'</span> ' : ' ').'<span style="font-size:12px;">'.to_currency($item_price).'</span>: '.$item_info->name.(($item_info->size)? ' ('.$item_info->size.')' : '')) : $item_info->name.(($item_info->size||$item_info->product_id)? ' ('.$item_info->product_id.' '.$item_info->size.')' : '') , 'id'=> $barcode_number);
			}
			else
			{				
