<?php
    ...
    global $global_breadcrumb;
		...
		if ($ci->uri->segment(1) == 'customers')
		{
			...
			if($ci->uri->segment(2) == 'birthday')
			{
				$return.=create_current_page_url(lang('common_birth_day'));
			}
			
			if($ci->uri->segment(2) == 'balance')
			{
				$return.=create_current_page_url(lang('common_balance'));
			}
			
			if($ci->uri->segment(2) == 'tmp_list')
			{
				$return.=create_current_page_url(lang('common_temp_list'));
			}
			if($ci->uri->segment(2) == 'type_list')
			{
				$return.=create_current_page_url('Loại khách hàng');
			}
			
			if($ci->uri->segment(2) == 'quotes_contract')
			{
				$return.=create_current_page_url('Mẫu văn bản');
			}
			if($ci->uri->segment(2) == 'categories')
			{
				$return.=create_current_page_url('Quản lý danh mục');
				
			}
			if($ci->uri->segment(2) == 'quotes_contract_view')
			{
				if ($ci->uri->segment(3) == -1)
				{
					$return.=create_current_page_url('Thêm mẫu văn bản');
				}
				else
				{
					$return.=create_current_page_url('Cập nhật mẫu văn bản');
				}
			}
			
			if($ci->uri->segment(2) == 'quotes_contract_type_list')
			{
				$return.=create_current_page_url('Loại mẫu văn bản');
			}
			
			if($ci->uri->segment(2) == 'quotes_contract_type_view')
			{
				if ($ci->uri->segment(3) == -1)
				{
					$return.=create_current_page_url('Thêm loại mẫu văn bản');
				}
				else
				{
					$return.=create_current_page_url('Cập nhật loại mẫu văn bản');
				}
			}
			...
			elseif($ci->uri->segment(2) == 'manage_sms_campain')
			{
				$return.=create_current_page_url('Quản lý chiến dịch SMS');
			}
			elseif($ci->uri->segment(2) == 'manage_sms_history_detail')
			{
				$return.=create_current_page_url('Lịch sử SMS');
			}
			elseif($ci->uri->segment(2) == 'manage_sms_history_input')
			{
				$return.=create_current_page_url('Lịch sử SMS');
			}
			elseif($ci->uri->segment(2) == 'manage_mail')
			{
				$return.=create_current_page_url('Quản lý mẫu email');
			}
			elseif($ci->uri->segment(2) == 'manage_group_send_SMS_email')
			{
				$return.=create_current_page_url('Quản lý nhóm SMS / Email');																														
				
			}
			elseif($ci->uri->segment(2) == 'manage_mail_campain')
			{
				$return.=create_current_page_url('Danh sách lịch gửi email');
			}
			elseif($ci->uri->segment(2) == 'manage_mail_history_input')
			{
				$return.=create_current_page_url('Lịch sử gửi mail');
				
			}
			else
			{
				$return.= create_customers_breadcrumb();
			}
			}elseif($ci->uri->segment(1) == 'contracts'){
			
			if($ci->uri->segment(2) == 'index' || $ci->uri->segment(2) == 'circle' || $ci->uri->segment(2) == 'expired')
			$page = $ci->uri->segment(4, 1);
  		elseif($ci->uri->segment(2) == 'view' || $ci->uri->segment(2) == 'email')
			$page = $ci->uri->segment(5, 1);
  		...
  		if($ci->uri->segment(3) == 'customer') {
  			...
  			$link_contract = base_url() . 'contracts/index/customer'.$page;
				
  			$return.='<a href="'.$link_list.'">Công việc dự án</a>';
				}elseif($ci->uri->segment(3) == 'supplier') {
  			$link_list = base_url() . 'suppliers';
  			$link_contract = base_url() . 'contracts/index/supplier'.$page;
				
  			$return.='<a href="'.$link_list.'">Nhà cung cấp</a>';
			}
			
  		if($ci->uri->segment(2) == 'index')
			$return.='<a href="'.$link_contract.'" class="current">Hợp đồng</a>';
  		elseif($ci->uri->segment(2) == 'view') {
  			$return.='<a href="'.$link_contract.'">Hợp đồng</a>';
				
  			if($ci->uri->segment(4) == -1)
				$return.='<a href="javascript:;" class="current">Thêm mới</a>';
  			else
				$return.='<a href="javascript:;" class="current">Cập nhật</a>';
				}elseif($ci->uri->segment(2) == 'circle') {
				$return.='<a href="javascript:;" class="current">Chu kỳ dịch vụ</a>';
        }elseif($ci->uri->segment(2) == 'expired') {
				$return.='<a href="javascript:;" class="current">Hợp đồng hết hạn</a>';
        }elseif($ci->uri->segment(2) == 'email') {
				$return.='<a href="'.$link_contract.'">Hợp đồng</a>';
				$return.='<a href="javascript:;" class="current">Gửi mail</a>';
			}
		}
		elseif($ci->uri->segment(1) == 'items')
		{
			...
			{
				$return.=create_current_page_url('Quản lý dịch vụ');
			}
			elseif($ci->uri->segment(2) == 'view_service')
			{
				$return .= '<a tabindex="-1" href="">Dịch vụ</a>';
				if($ci->uri->segment(3) == -1)
				$return.=create_current_page_url('Thêm mới');
				else
				$return.=create_current_page_url('Cập nhật');
			}
		...
		elseif($ci->uri->segment(1) == 'reports')
		{
			...
			elseif($ci->uri->segment(2) == 'bao_cao_thu_chi_chi_tiet_khach_hang')
			{
				$return.=create_report_breadcrumb('Báo cáo thu chi chi tiết khách hàng');
			}
			elseif($ci->uri->segment(2) == 'bao_cao_thu_chi_chi_tiet_nha_cung_cap')
			{
				$return.=create_report_breadcrumb('Báo cáo thu chi chi tiết nhà cung cấp');
			}
			elseif($ci->uri->segment(2) == 'bao_cao_tong_hop_thu_chi_noi_bo')
			{
				$return.=create_report_breadcrumb('Báo cáo tổng hợp thu chi nội bộ');
			}
			elseif($ci->uri->segment(2) == 'bao_cao_TONG_HOP_cong_no_khach_hang')
			{
				$return.=create_report_breadcrumb('Báo cáo tổng hợp công nợ khách hàng');
			}
			elseif($ci->uri->segment(2) == 'bao_cao_chi_tiet_cong_no_nhom_khach_hang')
			{
				$return.=create_report_breadcrumb('Báo cáo chi tiết công nợ nhóm khách hàng');
			}
			elseif($ci->uri->segment(2) == 'bao_cao_TONG_HOP_cong_no_nha_cung_cap')
			{
				$return.=create_report_breadcrumb('Báo cáo tổng hợp công nợ nhà cung cấp');
			}
			elseif($ci->uri->segment(2) == 'bao_cao_tong_hop_cong_no_nhom_khach_hang')
			{
				$return.=create_report_breadcrumb('Báo cáo tổng hợp công nợ nhóm khách hàng');
			}
			elseif($ci->uri->segment(2) == 'shift_expenses')
			{
				$ci->load->model('Category');
				$shift_category_id       = $ci->config->item('shift_category_id');
				if($shift_category_id > 0) {
					$category = $ci->Category->getItem($shift_category_id);
				}
				
				$return.=create_report_breadcrumb('Báo cáo ' . mb_strtolower($category['name'],'UTF-8'));
			}
			elseif($ci->uri->segment(2) == 'sales_generator')
			{
				$return.=create_current_page_url(lang('reports_sales_generator'));
			}		
			elseif($ci->uri->segment(2) == 'graphical_summary_customers')
			{
				$return.=create_report_breadcrumb(lang('reports_graphical_reports'));
			}
			elseif($ci->uri->segment(2) == 'summary_customers')
			{
				$return.=create_report_breadcrumb(lang('reports_summary_reports'));
			}			
			elseif($ci->uri->segment(2) == 'specific_customer' || $ci->uri->segment(2) == 'specific_cus')
			{
			...
			elseif($ci->uri->segment(2) == 'specific_employee' || $ci->uri->segment(2) == 'specific_employees')
			{
            ...
            elseif($ci->uri->segment(2) == 'bao_cao_chi_tiet_xuat_nhap_ton' || $ci->uri->segment(2) == 'bao_cao_chi_tiet_xuat_nhap_ton_store')
			{
				$return.=create_report_breadcrumb(lang('reports_detail_stock_movement_summary_report'));		
			}
			...
			elseif($ci->uri->segment(2) == 'specific_customer_store_account' || $ci->uri->segment(2) == 'specific_customer_store_acc')
			{
				$return.=create_report_breadcrumb(lang('reports_detailed_store_accounts_report'));
			}
			elseif($ci->uri->segment(2) == 'specific_supplier_store_acc')
			{
				$return.=create_report_breadcrumb('Báo cáo chi tiết công nợ NCC');
			}
			...
			elseif($ci->uri->segment(2) == 'detailed_commissions' || $ci->uri->segment(2) == 'detail_commissions')
			{
		...
		elseif ($ci->uri->segment(1) == 'sales')
		{
			...
			{
				$sales_home_link = '<a class="current" tabindex = "-1" href="'.site_url('sales/suspended').'">'.lang('module_sales').'</a>';
			...
			{
				$sales_home_link = '<a tabindex = "-1" href="'.site_url('sales/suspended').'">'.lang('module_sales_breadcrum').'</a>';
			}
			
			$return.=$sales_home_link;
			
			if($ci->uri->segment(2) == 'report_quotes_contract_all')
			{
				$return.=create_current_page_url('Xuất file');
			}
...
//			}
			...
			{
				$return.=create_current_page_url(lang('sales_receipt_service'));
			}
		...
		{
			if ($ci->uri->segment(2) == NULL || $ci->uri->segment(2) == 'shift') //Main page
			{
    ...
    elseif($ci->uri->segment(1) == 'tasks') {
			...
			if($ci->uri->segment(2) == 'personal') {
				...
				if ($ci->uri->segment(2) == NULL) //Main page
				{
					...
					$return.= '<a tabindex="-1" class="current" href="javascript:;">Danh sách mục dự án</a>';
	...
	function create_customers_breadcrumb()
	{
		$ci = &get_instance();
		
		$return = '';
		if ($ci->uri->segment(2) == 'view_mail') // Input page
		{
			$return.= '<a tabindex = "-1" href="'.site_url('customers/manage_mail/1').'">Quản lý mẫu mail</a>';	
			if($ci->uri->segment(3)>0)
			{
				$return.=create_current_page_url('Chỉnh sửa');
			}
			else
			{
				$return.=create_current_page_url('Thêm mới');
		...
		elseif ($ci->uri->segment(2) == 'view_mail_campain') // Input page
		{
			$return.= '<a tabindex = "-1" href="'.site_url('customers/manage_mail_campain/1').'">Quản lý chiến dịch mail</a>';	
			if($ci->uri->segment(3)>0)
			{
				$return.=create_current_page_url('Chỉnh sửa');
			}
		...
		elseif ($ci->uri->segment(2) == 'update_group_SMS_email') // Input page
		{
			$return.= '<a tabindex = "-1" href="'.site_url('customers/manage_group_send_SMS_email').'">Quản lý nhóm SMS / Email</a>';	
			if($ci->uri->segment(3)>0)
			{
				$return.=create_current_page_url('Chỉnh sửa');
			}
		...
		elseif ($ci->uri->segment(2) == 'manage_group_SMS_email_detail') // Input page
		{
			$return.= '<a tabindex = "-1" href="'.site_url('customers/manage_group_send_SMS_email').'">Quản lý nhóm SMS / Email</a>';	
			$return.=create_current_page_url('Chi tiết');
		}
		elseif($ci->uri->segment(2) == 'view_sms')
		{
			if($ci->uri->segment(3)>0)
			{
				$return.= '<a tabindex = "-1" href="'.site_url('customers/manage_sms').'">Quản lý mẫu SMS</a>';	
				$return.=create_current_page_url('Chỉnh sửa');
			}
			else
			{
		...
		elseif($ci->uri->segment(2) == 'view_sms_campain')
		{
			if($ci->uri->segment(3)>0)
			{
				$return.= '<a tabindex = "-1" href="'.site_url('customers/manage_sms_campain').'">Quản lý chiến dịch SMS</a>';	
				$return.=create_current_page_url('Chỉnh sửa');
			}
			else
			{
