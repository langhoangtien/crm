...
function get_people_manage_table($people,$controller)
{
	...
	if($controller_name=='customers')
	{
		...
		if ($CI->config->item('enable_customer_loyalty_system'))
		{
			...
			{
				$headers[] = '<span class="points_cats">' . lang('common_points').' / '.lang('customers_amount_to_spend_for_next_point') . '</span>';;							
			}
		}
		
		if ($CI->config->item('customers_store_accounts'))
		{
			$headers[] = $CI->config->item('customer_balance');
            $headers[] = $CI->config->item('customer_balance_2');
			$headers[] = '&nbsp;';
		}
...
function get_people_manage_table_data_rows($people,$controller)
{
	...
	{
		$table_data_rows.=get_person_data_row($person,$controller,$STT);
		$STT++;
	}
...
function get_person_data_row($person,$controller,$STT = 1)
{
    ...
    $link = base_url() . 'reports/specific_customer_store_acc/?cus_id='.$person->person_id;
	$table_data_row='<tr>';	
	...
	{	
		...
		if ($CI->config->item('enable_customer_loyalty_system'))
		{
			...
			{				
			   $sales_until_discount = $CI->config->item('number_of_sales_for_discount') - $person->current_sales_for_discount;
				$table_data_row.='<td><span class="underlines_2">'.to_quantity($sales_until_discount).'</span></td>';			
			}
		}
		...
		if ($CI->config->item('customers_store_accounts'))
		{
            ...
            $table_data_row.='<td>'.to_currency($person->balance_2).'</td>';
...
function get_supplier_manage_table($suppliers,$controller)
{
	...
	lang('suppliers_type'),
	lang('suppliers_focal'),
	lang('common_email'),
	lang('common_phone_number'),
	lang('suppliers_total_value'),
	lang('suppliers_update'),
	// $CI->config->item('supplier_balance'),
 //    $CI->config->item('supplier_balance_2'),
	'&nbsp;',
	'&nbsp;');
	$table.='<thead><tr>';
	$count = 0;
...
function get_supplier_manage_table_data_rows($suppliers,$controller)
{
	...
	$money = '<script src="'.base_url("assets/js/plugins/simple.money.format.js").'" type="text/javascript"></script>
	<script type="text/javascript">
	$(".total").simpleMoneyFormat();

	</script>';
	return $table_data_rows.$money;
}
...
function get_supplier_data_row($supplier,$controller)
{
    ...
    $link = base_url() . 'reports/specific_supplier_store_acc?supp_id=' . $supplier->person_id;
	$table_data_row.='<td>'.H($supplier->person_id).'</td>';
	$table_data_row.='<td><a href="'.$link.'">'. H($supplier->company_name). '</a></td>';
	$table_data_row.='<td>'.H($supplier->name).'</td>';
	$table_data_row.='<td>'.H($supplier->head).'</td>';
	$table_data_row.='<td>'.mailto(H($supplier->email),H($supplier->email)).'</td>';
	$table_data_row.='<td>'.H($supplier->phone_number).'</td>';		
    $table_data_row.='<td class="total">'.H(round($supplier->total)).'</td>';
...
function get_item_data_row($item,$controller)
{
	...
	$avatar_url=$item->image_id ?  site_url('app_files/view/'.$item->image_id) : base_url('assets/assets/images/items-default.jpg');
...
{ 
	...
	{
		$table_data_rows.=get_location_data_row($location,$controller, $STT);
		$STT++;
	}
...
function get_location_data_row($location,$controller,$STT = 1)
{
	$CI =& get_instance();
	$controller_name=str_replace(BIZ_PREFIX, '', strtolower(get_class($CI)));
...
function get_expenses_data_row($expense,$controller)
{
	...
	$table_data_row.='<td>'.$expense_type.'</td>';
...
function get_group_data_row($group, $controller = null)
{
    ...
    $table_data_row.='<td width="150px">'.H($group->employee_name).'</td>';
    $table_data_row.='<td width="150px">'.H($group->location_name).'</td>';
...
function get_department_data_row_employees($employees) {
    ...
    if (!empty($employees)) {
        foreach ($employees as $employee) {
            $data_row .= '<li><a href="'.base_url().'employees/view/'.$employee->person_id.'/2">' . $employee->first_name . ' ' . $employee->last_name . '</a> - '.$employee->group_name.'</li>';
        }
    }
    $data_row .= '</ul>';
    return $data_row;
...
function get_quotes_contract_data_row($quotes_contract, $controller) {
	...
	$table_data_row .= "<td width='15%'>$quotes_contract->code</td>";
	$table_data_row .= "<td width='41%'>$quotes_contract->title_quotes_contract</td>";
	$table_data_row .= "<td width='35%'>$quotes_contract->title</td>";
...
function get_mail_data_row($mail,$controller)
{
	...
	<input type='checkbox' id='mail_$mail->mail_id' value='" . $mail->mail_id . "' data-type='".$mail->mail_id."'/>
		<label for='mail_$mail->mail_id'><span></span></label>
	</td>";

	$table_data_row.='<td width="30%">'.$mail->mail_title.'</a></td>';
	$table_data_row.='<td width="50%">'.substr($mail->mail_content, 0, 20).'...</td>';	
	$table_data_row.='<td width="15%" class="rightmost">'.anchor("customers/view_mail/$mail->mail_id", lang('common_edit'),array('title'=>' Sửa mail')).'</td>';
	$table_data_row.='</tr>';
