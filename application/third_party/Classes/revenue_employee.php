...
$this->excel->getActiveSheet()->setCellValue('B4', "BÁO CÁO CHI TIẾT NHÂN VIÊN DOANH SỐ BÁN HÀNG NHÂN VIÊN");
...
$this->excel->getActiveSheet()->setCellValue('A6', "Tên nhân viên: " . mb_convert_case($employee_info->first_name . ' ' . $employee_info->last_name, MB_CASE_UPPER, "UTF-8"));
...
foreach ($data_all_sale as $key => $val) {
    foreach ($val as $key1 => $val1) {
        ...
        $this->excel->getActiveSheet()->setCellValue('D' . $k, $val1['first_name'] . ' ' . $val1['last_name']);
        foreach ($info_total_sale as $key2 => $val2) {
            if ($key2 == $key) {
                ...
                $this->excel->getActiveSheet()->setCellValue('E' . $k, format_quantity($total_item));
        ...
        $this->excel->getActiveSheet()->getStyle('F' . $k . ':J' . $k)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
...
if ($report_type == 'all') {
    ...
    $this->excel->getActiveSheet()->setCellValue('G' . ($j), 'Tổng doanh thu');
    ...
    $this->excel->setActiveSheetIndex(0)->mergeCells('F' . ($j) . ':G' . ($j));
    $this->excel->getActiveSheet()->setCellValue('F' . ($j), 'Tổng doanh thu');
    $this->excel->getActiveSheet()->getRowDimension($j)->setRowHeight(18);
    ...
    $this->excel->getActiveSheet()->setCellValue('H' . ($j), to_currency_unVND_nomar($total_cost) . ' VNĐ');
    $this->excel->getActiveSheet()->getStyle('H' . ($j))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

    $this->excel->setActiveSheetIndex(0)->mergeCells('F' . ($j + 1) . ':G' . ($j + 1));
    $this->excel->getActiveSheet()->setCellValue('F' . ($j + 1), 'Tổng chiết khấu');
    $this->excel->getActiveSheet()->getRowDimension($j + 1)->setRowHeight(18);
    ...
    $this->excel->getActiveSheet()->setCellValue('H' . ($j + 1), to_currency_unVND_nomar($total_discount) . ' VNĐ');
    $this->excel->getActiveSheet()->getStyle('H' . ($j + 1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

    $this->excel->getActiveSheet()->getStyle('F' . $j . ':F' . ($j + 2))->getFont()->setSize(12);
...
$filename = 'Báo cáo chi tiết doanh số bán hàng nhân viên.xlsx'; //save our workbook as this file name
