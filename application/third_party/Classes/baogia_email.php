...
$this->excel->getActiveSheet()->setCellValue('D2',"BẢNG BÁO GIÁ ");
...
$this->excel->getActiveSheet()->setCellValue('I3',"PN/12T");
...
//$this->excel->setActiveSheetIndex(0)->mergeCells('I10'.':I10');
$this->excel->setActiveSheetIndex(0)->mergeCells('G10'.':H10');
...
$this->excel->getActiveSheet()->getCell('G10')->setValue($objRichText13);
$this->excel->getActiveSheet()->getStyle('G10')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
  

//$this->excel->setActiveSheetIndex(0)->mergeCells('H11'.':I11');
...
$this->excel->getActiveSheet()->setCellValue('D14',' Ông/bà (Mr/Ms): '.mb_convert_case($data['customer'],MB_CASE_UPPER, "UTF-8"));
...
$this->excel->getActiveSheet()->setCellValue('D22',"Hình ảnh");
...
$this->excel->getActiveSheet()->setCellValue('D23',"(Image)");
...
//	die('ssss');
    ...
    //$this->excel->setActiveSheetIndex(0)->mergeCells('C'.$k.':C'.($k+1));	
    $this->excel->getActiveSheet()->setCellValue('C'.($k),$item['name']);
    $this->excel->getActiveSheet()->setCellValue('C'.($k+1),$item['description']);
    $this->excel->getActiveSheet()->getStyle('C'.($k+1))->getAlignment()->setWrapText(true);

    //$this->excel->setActiveSheetIndex(0)->mergeCells('D'.$k.':E'.($k+1));
    $this->excel->setActiveSheetIndex(0)->mergeCells('D'.$k.':E'.($k));
    $this->excel->setActiveSheetIndex(0)->mergeCells('D'.($k+1).':E'.($k+1));
    $this->excel->getActiveSheet()->setCellValue('D'.($k),$item['item_number']);
    ...
    //$this->excel->getActiveSheet()->setCellValue('D'.($k+1),$this->Item->get_info($item['item_id'])->images);
    if($item['item_kit_id'])
    {
        $anh = $this->Item_kit->get_info($item['item_kit_id'])->images;
        if($anh != "")
        {
            $imagePath = 'item_kits/'.$anh;
            $objDrawing = new PHPExcel_Worksheet_Drawing();
            $objDrawing->setPath($imagePath);
            $objDrawing->setHeight(75);
            $objDrawing->setWidth(90);
            $objDrawing->setCoordinates('D'.($k+1));
            $objDrawing->setWorksheet($this->excel->getActiveSheet());
            $this->excel->getActiveSheet()->getStyle('D'.($k+1))->getAlignment()->setIndent(5);
            //$this->excel->getActiveSheet()->getStyle('D'.($k+1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            //$this->excel->getActiveSheet()->getStyle('D'.($k+1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $this->excel->getActiveSheet()->getStyle('D'.$k)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $this->excel->getActiveSheet()->getStyle('D'.$k)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        }
        else
    ...
    $this->excel->getActiveSheet()->getStyle('C'.($k+1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $this->excel->getActiveSheet()->getStyle('C'.($k+1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		
    $this->excel->setActiveSheetIndex(0)->mergeCells('F'.$k.':F'.($k+1));
    if($this->Item->get_info($item['item_id'])->unit_rate == 0)
    {
        $this->excel->getActiveSheet()->setCellValue('F'.$k,$this->Unit->item_unit($this->Item->get_info($item['item_id'])->unit_from)->name);
    }
    else
    {
...
for($a = $k ;$a <= $k;$a++){
    ...
    //$this->excel->getActiveSheet()->getStyle('C'.$a)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
// $this->excel->getActiveSheet()->getStyle('F'.($k).':F40')->getBorders()->getLEFT()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
// $this->excel->getActiveSheet()->setCellValue('I'.($i+2),to_currency_unVND_nomar($data['total']);
// $this->excel->getActiveSheet()->getStyle('I'.($i+2))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
// foreach($data['payments'] as $payment_id=>$payment)  $data['payment_type']
 // {
...
//$a = substr($data['total'],0,-3);
$c = to_currency_unVND_nomar($data['total']);
//$a = substr($c,0,-3);
$a = str_replace(',','',$c);
//echo $c.'   ';
//echo $a.'   ';
...
$this->excel->getActiveSheet()->setCellValue('A'.($i+5),"CÁC ĐIỀU KHOẢN THƯƠNG MẠI: ");
...
// $this->excel->getActiveSheet()->setCellValue('B'.($i+6),"1.");
// $this->excel->getActiveSheet()->getStyle('B'.($i+6))->getAlignment()->setWrapText(true);
// $this->excel->getActiveSheet()->getStyle('B'.($i+6))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

$this->excel->setActiveSheetIndex(0)->mergeCells('B'.($i+6).':I'.($i+6));
$this->excel->getActiveSheet()->setCellValue('B'.($i+6),"1. Thời gian làm mẫu:");
$this->excel->getActiveSheet()->getStyle('B'.($i+6))->getAlignment()->setWrapText(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('B'.($i+7).':I'.($i+7));
$this->excel->getActiveSheet()->setCellValue('B'.($i+7),"2. Thời gian giao hàng:");
$this->excel->getActiveSheet()->getStyle('B'.($i+7))->getAlignment()->setWrapText(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('B'.($i+8).':I'.($i+8));
$this->excel->getActiveSheet()->setCellValue('B'.($i+8),"3. Địa điểm giao hàng: Giao hàng miễn phí trong phạm vi bán kính 10km tính từ VP công ty tại số 240 Đê La Thành, Đống Đa, Hà Nội");
$this->excel->getActiveSheet()->getStyle('B'.($i+8))->getAlignment()->setWrapText(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('B'.($i+9).':I'.($i+9));
$this->excel->getActiveSheet()->setCellValue('B'.($i+9),"4. Điều kiện thanh toán: Bên mua đặt cọc 50% tổng giá trị đơn hàng cho bên Bán khi ký kết đơn hàng. Ngay khi nhận hàng và hóa đơn chứng từ đầy đủ bên Mua thanh toán 50% tổng giá trị đơn hàng còn lại cho bên Bán.");
...
$this->excel->getActiveSheet()->setCellValue('B'.($i+10),"Tài khoản công ty: \n Chủ TK: CÔNG TY CỔ PHẦN QUÀ TẶNG QUẢNG CÁO VNBG \n Số TK: 2 111 0000 500 589 \n Ngân hàng BIDV, chi nhánh Hà Nội \n \n Tài khoản cá nhân VIB \n Chủ TK: Vũ Thị Hương \n Số TK: 0067 040 6007 8637 \n Ngân hàng VIB, chi nhánh Ba Đình");
$this->excel->getActiveSheet()->getStyle('B'.($i+10))->getAlignment()->setWrapText(true);
$this->excel->getActiveSheet()->getStyle('B'.($i+10))->getFont()->setSize(10)->setBold(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('D'.($i+11).':I'.($i+11));
$this->excel->getActiveSheet()->setCellValue('D'.($i+11),"Cảm ơn Quý khách hàng, mong sự cộng tác bền lâu!");
$this->excel->getActiveSheet()->getStyle('D'.($i+11))->getAlignment()->setWrapText(true);
$this->excel->getActiveSheet()->getStyle('D'.($i+11))->getFont()->setItalic(true);
$this->excel->getActiveSheet()->getStyle('D'.($i+11))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

$this->excel->setActiveSheetIndex(0)->mergeCells('A'.($i+12).':E'.($i+12));
$this->excel->getActiveSheet()->setCellValue('A'.($i+12),"QUÀ TẶNG SỰ KIỆN ENSA");
$this->excel->getActiveSheet()->getStyle('A'.($i+12))->getAlignment()->setWrapText(true);
$this->excel->getActiveSheet()->getStyle('A'.($i+12))->getFont()->setBold(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('F'.($i+12).':I'.($i+12));
$this->excel->getActiveSheet()->setCellValue('F'.($i+12),"Xác nhận của quý khách hàng");
$this->excel->getActiveSheet()->getStyle('F'.($i+12))->getAlignment()->setWrapText(true);
$this->excel->getActiveSheet()->getStyle('F'.($i+12))->getFont()->setBold(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('A'.($i+13).':E'.($i+13));
$this->excel->getActiveSheet()->setCellValue('A'.($i+13),"(Giám Đốc)");
$this->excel->getActiveSheet()->getStyle('A'.($i+13))->getAlignment()->setWrapText(true);
$this->excel->getActiveSheet()->getStyle('A'.($i+13))->getFont()->setItalic(true);

$this->excel->setActiveSheetIndex(0)->mergeCells('F'.($i+13).':I'.($i+13));
$this->excel->getActiveSheet()->setCellValue('F'.($i+13),"(Ký và ghi rõ họ tên)");
$this->excel->getActiveSheet()->getStyle('F'.($i+13))->getAlignment()->setWrapText(true);
$this->excel->getActiveSheet()->getStyle('F'.($i+13))->getFont()->setItalic(true);

$this->excel->getActiveSheet()->getStyle('A'.($i+12).':I'.($i+13))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$this->excel->getActiveSheet()->getStyle('A'.($i+12).':I'.($i+13))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
 /* */
/* */
...
$this->excel->getActiveSheet()->getRowDimension($i+8)->setRowHeight(31.25);
$this->excel->getActiveSheet()->getRowDimension($i+9)->setRowHeight(47.25);
$this->excel->getActiveSheet()->getRowDimension($i+10)->setRowHeight(120);
...
$this->excel->getActiveSheet()->getColumnDimension('D')->setWidth(14.9);
$this->excel->getActiveSheet()->getColumnDimension('E')->setWidth(0);
