...
<div class="row">
	...
	{
		$('#giftcard_number').on('change', function(){
			var giftcard_number = $(this).val();
			var reg = /^[1-9]{1}[0-9]*$/;
			var test1 = giftcard_number.substring(0,1);
			var test = reg.test(giftcard_number);
			
			if (test1 == 0) {
				toastr.error('Mã thẻ không được bắt đầu là số 0.', 'error');
				$('#submit').addClass('disabled');
			} else if (test === false) {
				toastr.error('Mã thẻ không được nhập là chữ.', 'error');
				$('#submit').addClass('disabled');
			} else {
		...
		$('#giftcard_form').validate({
			...
			messages:
			{
				giftcard_number:
				{
					...
					// number:<?php echo json_encode(lang('common_giftcards_number')); ?>,
