...
<div class="container">
    <h3>
        TH/KH Doanh thu HĐTV VCBS giao
    </h3>
...
<table class="table" id="table_kpi_profit">
    ...
    <tr>
        ...
        <td class="" style="text-align: center;" colspan="5">TH/KH doanh thu HĐTV VCBS giao</td>
        <td class="" style="text-align: center;" colspan="5">TH/KH lợi nhuận HĐTV VCBS giao</td>
    </tr>
    <tr>
    ...
    if ($location) {
        ...
        foreach ($location as $key => $value) {
            $value_data = json_decode($kpi_thkh_hdtv_convert[$key]);
            $total_profit = 0;
            $total_revenue = 0;
            echo '<tr>
                     <td width="9%">' . $value . '</td>';
                     for ($i=1; $i < 5; $i++) { 
                      if($view_data[$key]['revenue'][$i-1]['value']>0)
                      $revenue = ($data_revenue_profit[$key][$i]['revenue']/$view_data[$key]['revenue'][$i-1]['value'])*100;
                    else $revenue = 0;
                     ...
                     for ($i=1; $i <5 ; $i++) { 
                         if($view_data[$key]['profit'][$i-1]['value']>0)
                          $profit = ($data_revenue_profit[$key][$i]['profit']/$view_data[$key]['profit'][$i-1]['value'])*100;
                        else $profit =0;
