<table class="table table-hover mt-10">
    ...
    <tr>
        ...
        <th>
            <select old_value="" onfocus="this.old_value = this.value" onchange="return action_select_column($(this))" name="columns[<?php echo $column; ?>]" class="form-control item-form">
                ...
                if(!empty($location_items)) {
                    ...
                    <optgroup label="Số lượng">
                        <?php
                        foreach($location_items as $val) {
                            ?>
                            <option value="quantity:<?php echo $val['location_id']; ?>"><?php echo $val['name']; ?></option>
                        <?php
                        }
                        ?>
    ...
    <tr data-import-id="<?php echo $index; ?>">
        ...
        <td <?php if ($index > 1) :?>class="selected"<?php endif; ?>>
            ...
            <input name="rows[<?php echo ($index); ?>][<?php echo $column; ?>]" type="text" class="form-control item-form" value="<?php echo $value; ?>" />
            <?php unset($value); ?>
        </td>
        <?php $column_index++; endforeach; ?>
        <td <?php if ($index > 1) :?>class="selected"<?php endif; ?>>
            <button type="button" onclick="$(this).parent().parent().remove()" class="btn btn-sm red-thunderbird"><?php echo lang('common_delete'); ?></button>
        </td>
    </tr>
        <?php endfor; ?>
    </tbody>
