<div class="modal fade" id = "itemsModal" tabindex="-1" role="dialog">
	<div class="modal-dialog modal-lg customer-recent-sales" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="panel row form-group modal-item-info">
                ...
                <?php if (!$isInItemContainsLine && $this->config->item('company_user') == 'remHaMy'):?>
			    <?php $this->load->view('attribute_sets/widgets-items/attribute_set_items', array('entity_info' => $item_info)); ?>
				<div id="attribute_sets">

                </div>
                <?php elseif ($this->config->item('company_user') == 'remHaMy'):?>
                <div class="panel row form-group modal-item-info">
                    <span class="modal-item-name">Bạn phải nhóm sản phẩm mới có thể thay đổi thuộc tính</span>
				...
				<table class="table table-bordered table-hover table-striped">
					...
					<tr> <td><?php if ($this->config->item('company_user') == 'remHaMy'){ echo 'Khổ vải'; } else { echo lang('common_size');}; ?></td> <td> <?php echo H($item_info->size); ?></td></tr>
					...
					<?php } ?>
					<tr> <td><?php echo lang('common_unit_price'); ?></td> <td> <?php echo to_currency($item_info->unit_price, 10); ?></td></tr>
					<?php 
				...
				<table class="table table-bordered table-hover table-striped" width="1200px">
					<tr>
						<td colspan="2"><h3><?php echo lang('sales_list_of_suspended'); ?></h3></th>
					</tr>
					<tr>
						<th><?php echo lang('receivings_id');?></th>
						<th><?php echo lang('items_quantity');?></th>
					...
					<?php foreach($suspended_sales as $sale_item) {?>
						<tr>
							<td style="text-align: center;"><?php echo anchor('sales/receipt/'.$sale_item['sale_id'], 'HDBH '.$sale_item['sale_id'], array('target' => '_blank'));?></td>
							<td style="text-align: center;"><?php echo to_quantity($sale_item['quantity_purchased']);?></td>
						</tr>
					<?php } ?>
				</table>
			</div>
