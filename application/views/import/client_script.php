<script type="text/javascript">
    ...
    function action_import_data(container_id) {
        ...
        $.ajax({
            ...
            success: function(response) {
                response = $.parseJSON(response);
                var success = response.success_rows;
                var errors  = response.error_rows;
                if(!response.success) {
                    show_feedback('error', null, response.msg);
                }

                if(success.length) {
                    $.each(success, function( index, row_id ) {
                        $('#data-import table tr[data-import-id="'+row_id+'"]').remove();
                    });
                    toastr.success('Import thành công ' +success.length+' bản ghi.' , 'Thông báo!');
                }

                if(errors.length) {
                    $.each(errors, function( index, row_id ) {
                        $('#data-import table tr[data-import-id="'+row_id+'"]').addClass('error');
                    });

                    toastr.error(errors.length+' bản ghi không được xử lý' , response.error_message);
                }
