...
class Deleted_sales extends Report
{
	...
	public function getData()
	{
		...
		$this->db->select(' locations.name as location_name,customerData.account_number as account_number,sale_id, sale_time, sale_date,register_name, sum(quantity_purchased) as items_purchased, CONCAT(sold_by_employee.first_name," ",sold_by_employee.last_name) as sold_by_employee, CONCAT(employee.first_name," ",employee.last_name) as employee_name, CONCAT(customer.first_name," ",customer.last_name) as customer_name,CONCAT(deleted_by.first_name," ",deleted_by.last_name) as deleted_by, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, payment_type, comment', false);
		...
		$this->db->join('customers as customerData', 'sales_items_temp.customer_id = customerData.person_id', 'left');
