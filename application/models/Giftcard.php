...
class Giftcard extends CI_Model
{
	...
	*/
	...
	function log_modification($data)
	{
		...
		} elseif ($data['type'] == 'sale_return') {
            $spent = to_currency($transaction_amount);
			$new_value = to_currency($row['value']);
            $log_message = lang('sales_return_id'). ': '.anchor('sales/receipt/'.$data['sale_id'], $this->config->item('sale_prefix'). ' '.$data['sale_id'], array('target' => '_blank')).' '.$data['person'].' '.lang('giftcards_added').' '.$spent. " ".lang('giftcards_with_a_new_value_of')." ". $new_value;
		...
		elseif ($data['type'] == 'create')
		{
			...
			$log_message = $sale_id_message.$data['person']." ".lang('giftcards_created_with_value')." ".to_currency($transaction_amount);
		}
