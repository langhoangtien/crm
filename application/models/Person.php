	...
	function save(&$person_data,$person_id=false)
	{
		if (!$person_id or !$this->exists($person_id))
		{
			if ($this->db->insert('people',$person_data))
			{
				...
				# Lưu số thứ tự mới tăng 1 đơn vị
				if(isset($person_data['iValid']) && $person_data['iValid'] && isset($_SESSION['ma_khach_hang_bat_dau_tu'])){
					$_SESSION['ma_khach_hang_bat_dau_tu'] = $_SESSION['ma_khach_hang_bat_dau_tu'] + 1;
				}
