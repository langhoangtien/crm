...
class Item extends Bizmodel
{
	...
	*/
	function get_all($limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		$this->db->select('items.*, item_images.image_id as image_id, GROUP_CONCAT('.$this->db->dbprefix('tags').'.name) as tags, categories.name as category,
		categories.id as category_id, measures.name as measure_name,
		...
		$this->db->join('measures', 'items.measure_id = measures.id', 'left');
	...
	function get_all_by_category($category_id, $hide_out_of_stock_grid = FALSE, $offset=0, $limit = 14)
	{
		...
		{
			$result = $this->db->query("(SELECT item_id, unit_price, name, size, $items_table.image_id FROM $items_table LEFT JOIN $item_images_table USING (item_id)
	...
	function get_all_by_tag($tag_id, $hide_out_of_stock_grid = FALSE, $offset=0, $limit = 14)
	{
		...
		if (!$hide_out_of_stock_grid)
		{
			$result = $this->db->query("(SELECT item_id, unit_price,name, $items_images_table.image_id FROM $items_table LEFT JOIN $items_images_table USING (item_id) INNER JOIN $items_tags_table USING (item_id)
	...
	{
		$items_table = $this->db->dbprefix('items');
		$result = $this->db->query("SELECT item_id FROM $items_table WHERE item_id = (select min(item_id) from $items_table where deleted = 0 and item_id > ".$this->db->escape($item_id)." and name <> 'Giảm giá' and name <> 'Thanh toán công nợ')");
	...
	{
		$items_table = $this->db->dbprefix('items');
		$result = $this->db->query("SELECT item_id FROM $items_table WHERE item_id = (select max(item_id) from $items_table where deleted = 0 and item_id <".$this->db->escape($item_id)." and name <> 'Giảm giá' and name <> 'Thanh toán công nợ')");
	...
	function getInformation($arrParam) {
		$this->db->select('*')
		         ->from('items')
			     ->where('item_id',$arrParam['item_id_or_number'])
			     ->or_where('item_number',$arrParam['item_id_or_number']);
		$query = $this->db->get();
	...
	*/
	function get_info($item_id, $can_cache = TRUE)
	{
		...
		$this->db->select('*,measures.name as don_vi_tinh,items.name as name ,categories.id as category_id, categories.name as category_name')
						 ->from('items')
						 ->join('categories', 'items.category_id = categories.id','left')
						 ->join('measures', 'measures.id = items.measure_id','left');
		$this->db->where('item_id',$item_id);
		...
		else
		{
			...
			//Get all the fields from sales_items table
			$fields = $this->db->list_fields('sales_items');
	...
	*/
	function get_multiple_info($item_ids)
	{
		...
		$this->db->select('*,measures.name as don_vi_tinh,items.name as name ,categories.name as category_name')
						 ->from('items')
						 ->join('categories', 'items.category_id = categories.id','left')
	...
	*/
    ...
    /*
     update item by any field
     */
    function update_item($field, $value, $data) {
        unset($data[$field]);
    ...
    public function get_item_by_field($field, $value) {
        $this->db->select("*")
                ->from('items')
                ->where($field,$value);
	...
	*/
	function get_manage_items_search_suggestions($search,$limit=25)
	{
		...
		if($this->config->item('supports_full_text') && !$this->config->item('legacy_search_method'))
		{
			...
			$this->db->select("categories.name as category, item_images.image_id, items.item_id as item_id, items.size as size, items.name as item_name, MATCH (".$this->db->dbprefix('categories').".name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel", false);
			$this->db->from('categories');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			...
			$this->db->select("item_number,item_id,item_images.image_id,items.name, categories.name as category, size, MATCH (item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel", false);
			$this->db->from('items');
			$this->db->join('categories', 'categories.id = items.category_id','left');
			...
			$this->db->select("item_id,item_images.image_id,items.name, categories.name as category, size, MATCH (item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel", false);
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
			...
			$this->db->select("additional_item_numbers.item_number,item_images.image_id, items.name,items.item_id, MATCH (".$this->db->dbprefix('additional_item_numbers').".item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel", false);
			...
			$this->db->select("tags.name as tag_name, items.name as name, items.item_id as item_id, item_images.image_id, MATCH (".$this->db->dbprefix('tags').".name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel", false);
			$this->db->from('items_tags');
	...
	function check_duplicate($term)
	{
		$this->db->from('items');
		$this->db->where('deleted',0);		
		$this->db->where("name = ".$this->db->escape($term));
		$query=$this->db->get();
		if($query->num_rows()>0)
		{
			return true;
	...
	function get_item_search_n9($search,$limit=25) {
		if (!trim($search))
		{
			return array();
		}
	...
	public function getTotalQty($item_ids = [])
	{
	    
	    $this->db->select('items.item_id, SUM('. $this->db->dbprefix('location_items') .'.quantity) as total_quantity');
	...
	*/
	...
	function search($search, $category_id = false, $limit=20,$offset=0,$column='name',$orderby='asc', $fields = 'all')
	{
		...
		if ($category_id)
		{
		    if (is_array($category_id)) {
		        $this->db->where_in('categories.id', $category_id);
		    } else {
		        $this->db->where('categories.id', $category_id);
		        $this->db->or_where('categories.parent_id', $category_id);
		    }
		}
		
		if (!empty($this->options['search']['options']['low_inventory'])) {
		    $this->db->where("((CASE WHEN ". $this->db->dbprefix('location_items') .".reorder_level IS NULL THEN ". $this->db->dbprefix('items') .".reorder_level ELSE ". $this->db->dbprefix('location_items') .".reorder_level END) > ". $this->db->dbprefix('location_items') .".quantity)");
		}
	...
	function cleanup()
	{
		...
		// return $this->db->update('items',$item_data);
		return $this->db->delete('items',$item_data);
	}
