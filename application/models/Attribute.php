...
class Attribute extends CI_Model
{
    ...
    */
    ...
    const ATTRIBUTE_TYPE_FORMULA  = 9;
    ...
    */
    public function get_types()
    {
        ...
        $attribute_types[self::ATTRIBUTE_TYPE_FORMULA]  = lang('attributes_type_formula');
    ...
    */
    ...
    public function get_row($attribute)
    {
        ...
        $row .= '<td width="450px">' . H($attribute->name) . '</td>';
    ...
    public function get_type_html($attribute)
    {
        switch ($attribute->type) {
            ...
            case self::ATTRIBUTE_TYPE_FILE:
                $type = 'file';
                break;
            case self::ATTRIBUTE_TYPE_FORMULA:
                $type = 'formula';
                break;
            default:
                $type = 'text';
    ...
    public function get_entity_attributes($data)
    {
        ...
        $attributes = array();
        if(!empty($data['key_code']) && $data['key_code']) {
            foreach ($collection as $attribute) {
                $attributes[$attribute->code] = $attribute->entity_value;
            }   
        }
        else {
            foreach ($collection as $attribute) {
    ...
    */
    ...
    public function get_attribute_id_by_codes($codes)
    {
        $this->db->select('*');
        $this->db->from('attributes');
