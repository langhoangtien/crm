...
class Item_location extends CI_Model
{
	...
	function update_chuyen_kho_noi_bo($quantity, $item_id, $location_id=false, $is_insert = false)
	{
		$result = false;
    	...
    	if($is_insert)
		{
			...
			$result = $this->db->insert('location_items',$data);
		}
		else
		{
			$this->db->where('item_id', $item_id);
			...
			$result = $this->db->update('location_items',$data);
		}
