...
class Receiving extends CI_Model
{
    protected $_fields = array(
        ...
        'receiving_time'   => 'r.receiving_time',
    );

    protected $_receiving_tmp_ids = array();
	...
	{
		$this->db->where('receiving_id',$receiving_id);
		$this->db->update('store_supplier_accounts',array('date'=>$receiving_data['receiving_time']));
	...
	function save ($items,$supplier_id,$employee_id,$comment,$payment_type,$receiving_id=false, $suspended = 0, $mode='receive',$change_receiving_date = false, $is_po = 0, $location_id=-1, $return = 0, $store_account_payment = 0, $is_vat = 0, $suppl_info = NULL)
	{
		if(count($items)==0)
			return -1;
		...
		foreach($items as $line=>$item)
		{
			...
			if (!$cur_item_info->is_service)
			{
				...
				if ($inventory_to_add !=0)
				{
					...
					$recv_remarks ='Nhập hàng trực tiếp cho nhà cung cấp '.$receiving_id;
					$inv_data = array
					(
	...
	function delete($receiving_id, $all_data = false, $update_quantity = true)
	{
		...
		if ($all_data)
		{
			...
			$this->update_store_supplier_account($receiving_id);
	...
	function undelete($receiving_id)
	{
		...
		$this->update_store_supplier_account($receiving_id,1);
		$this->db->where('receiving_id', $receiving_id);
		return $this->db->update('receivings', array('deleted' => 0,'deleted_by'=>NULL));
	}
	function update_store_supplier_account($receiving_id,$undelete=0)
		{
			//update if Store account payment exists
			$this->db->from('store_supplier_accounts');
			$this->db->where('receiving_id',$receiving_id);
			$results = $this->db->get();
				...
				foreach($results->result() as $result)
				{
					if($result->transaction_amount)
					{
						...
						if($undelete==0)
						{
							if($result->options == 2)
							{
								$this->db->set('balance_2','balance_2-'.$result->transaction_amount,false);
							}
							else
							{
								$this->db->set('balance','balance-'.$result->transaction_amount,false);
							}
						...
						$this->db->where('person_id', $result->supplier_id);
						$this->db->update('suppliers');
						if($undelete==0)
						{
							$this->db->set('comment',lang('receivings_delete_entire_receiving'));
							$this->db->where('receiving_id', $receiving_id);
							$this->db->update('store_supplier_accounts');
						}
						else
						{
							if($result->options == 2)
							{
								$this->db->set('comment',lang('receivings_balances'));
								$this->db->where('receiving_id', $receiving_id);
								$this->db->update('store_supplier_accounts');
							}
							else
