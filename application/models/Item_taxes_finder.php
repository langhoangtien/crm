	...
	*/
	function get_info($item_id, $transaction_type = 'sale') //Can be sale or receiving
	{
		if($transaction_type == 'receiving')
		{
			...
			$return = array();
			$default_receiving_VAT      = $this->config->item('default_receiving_VAT');
		  $default_receiving_VAT_name = $this->config->item('default_receiving_VAT_name');
			if (is_numeric($default_receiving_VAT))
			{
				$return[] = array(
					'id' => -1,
					'item_id' => $item_id,
					'name' => $default_receiving_VAT_name,
					'percent' => $default_receiving_VAT,
					'cumulative' => 0
				);
			}
			return $return;
