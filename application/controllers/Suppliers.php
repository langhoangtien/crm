...
class Suppliers extends Person_controller
{
    ...
    function index($offset = 0)
    {
        ...
        // $this->load->view('people/suppliers', $data);
        $data['all_unit_type'] = $this->Supplier->get_all_unit_type();
        $data['suppliers'] = $table_data->result_array();
    ...
    */
    ...
    function excel_export()
    {
        ...
        $this->load->helper('download');
        // var_dump(DIRECTORY_SEPARATOR);die();
    ...
    */
...

    {
        ...
        $data['unit_type'] =array();
        $data['unit_type'][''] ='Chọn loại đơn vị';
        $unit_type = $this->Supplier->get_unit_type();
         if (count($unit_type) > 0){
            foreach ($unit_type as  $value) {
                $data['unit_type'][$value['id']] = $value['name'];
            }
        }
        $data['company_form'] = array();
        $data['company_form'][''] = "Chọn hình thức công ty";
        $company_form = $this->Supplier->get_company_form();
         if (count($company_form) > 0){
            foreach ($company_form as  $value) {
                $data['company_form'][$value['id']] = $value['name'];
            }
        }

        $danh_muc_tong_hop = $this->Supplier->get_danh_muc_ben_thu_ba(false, false, $iValid = TRUE);
    
           foreach ($danh_muc_tong_hop as $key => $value) {
            foreach ($value as $name) {
                $has_access = $this->Supplier->kiem_tra_danh_muc_supplier($key, $supplier_id, $name['id']);
        ...
        $data['thong_tin_lien_he'] = $this->Supplier->lay_thong_tin_lien_he_them($supplier_id);
        // var_dump($data['thong_tin_lien_he'] );die();
        if(!empty($data['thong_tin_lien_he']['birth_date_more']))
        {
        $data['thong_tin_lien_he']['birth_date_more']=date('d-m-Y',strtotime($data['thong_tin_lien_he']['birth_date_more']));
        }
        $data['thong_tin_dau_moi'] =  $this->Supplier->lay_thong_tin_dau_moi_them($supplier_id);
    ...
    function save($supplier_id = -1)
    {
        ...
        } else //failure
        {
            ...
            echo json_encode(array('success' => false, 'message' => lang('suppliers_error_adding_updating') . ' ' .
                $supplier_data['company_name'], 'person_id' => -1 , 'dkm' => 'vao day'));
        }
    }
    ...
    */
    ...
    function deletes() {
        $this->check_action_permission('delete');
        $suppliers_to_delete = $this->input->post('items');

        $this->Supplier->delete_list($suppliers_to_delete);
    }
