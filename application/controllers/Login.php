...
class Login extends MY_Controller
{
    ...
    function index()
    {
        ...
        //     $data['captcha_error'] = $_SESSION['captcha/error'];
        // }
        if (!$this->input->is_cli_request()) {
            if ($this->Employee->is_logged_in()) {
                ...
                //     if (empty($secret)) {
                //         $_SESSION['captcha/error'] = 'Thiếu mã bảo mật hệ thống';
                //         if (!empty($refer_url)) {
                //             return redirect($refer_url);
                //         }
                //     }
                //     if (empty($response)) {
                //         $_SESSION['captcha/error'] = 'Xác nhận bảo mật sai, bạn vui lòng thử lại';
                //         if (!empty($refer_url)) {
                //             return redirect($refer_url);
                //         }
                //     }
                ...
                //     $result = $google_api->validate_captcha($response);
    ...
    function edit_profile()
    {
        ...
        $data['check'] = $this->Employee->has_module_action_permission('employees', 'add_update', $this->Employee->get_logged_in_employee_info()->person_id);
    ...
    function do_edit_profile()
    {
        ...
        $check = $this->Employee->has_module_action_permission('employees', 'add_update', $this->Employee->get_logged_in_employee_info()->person_id);
        ...
        if ($this->input->post('password') != '') {
            ...
            if (!$check) {
                $employee_data['username'] = $this->Employee->get_logged_in_employee_info()->username;
            }
