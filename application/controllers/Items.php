...
class Items extends Secure_area implements Idata_controller
{
    ...
    function categories_group(){
        $this->load->view('items/categories_group');
            ...
            foreach ($tree as $node) {
                if($shift_category_id != $node->id) {
                    $return .= '<li>' . $node->name . ' <a href="javascript:void(0);" class="add_child_category" data-category_id="' . $node->id . '">[' . lang('items_add_child_category') . ']</a> ' .
                        ...
                        '&nbsp;&nbsp;&nbsp;<input type="checkbox" ' . ($node->hide_from_grid ? 'checked="checked"' : '') . ' class="hide_from_grid" id="hide_from_grid_' . $node->id . '" value="1" name="hide_from_grid_' . $node->id . '" data-category_id="' . $node->id . '" />';

                }else {
                    $return .= '<li>' . $node->name . '' .
                        '' .
                        '' .
                        '&nbsp;&nbsp;&nbsp;<label for="hide_from_grid_' . $node->id . '">' . lang('items_hide_from_item_grid') . '</label> <input type="checkbox" ' . ($node->hide_from_grid ? 'checked="checked"' : '') . ' class="hide_from_grid" id="hide_from_grid_' . $node->id . '" value="1" name="hide_from_grid_' . $node->id . '" data-category_id="' . $node->id . '" />';
    ...
    */
    ...
    function view($item_id = -1, $redirect = 0, $sale_or_receiving = 'sale')
    {
        ...
        if ($this->input->post('change_attribute') != 0 )
        {
            echo $data;
        } 
        else {
    ...
    function bulk_update()
    {
        ...
        foreach ($_POST as $key => $value) {
            ...
            if ($key == 'is_service' && $value == 1) {
                $this->Item_location->update_multiple(array('quantity' => NULL), $items_to_update, $select_inventory);
            } elseif ($key == 'is_service' && $value != 1) {
    ...
    function delete()
    {
        ...
        $items_to_delete = $this->input->post('cid');
        $select_inventory = $this->get_select_inventory();
        $params = $this->session->userdata('item_search_data') ? $this->session->userdata('item_search_data') : array('offset' => 0, 'order_col' => 'name', 'order_dir' => 'asc', 'search' => FALSE, 'category_id' => FALSE, 'fields' => 'all');
        ...
        if ($this->Item->delete_list($items_to_delete, $select_inventory)) {
            echo json_encode(array('flag'=>'true','msg' => lang('items_successful_deleted') . ' ' .
            $total_rows . ' ' . lang('items_one_or_multiple')));
        } else {
            echo json_encode(array('flag'=>'warning','msg' => lang('items_cannot_be_deleted')));
        }
    }
    ...
    */
            function excel()
    {
        ...
        $bizExcel = new BizExcel('import_newitems.xlsx');
        $content = $bizExcel->generateFile(false);
        force_download('import_newitems.'.($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'), $content);
    }
    ...
    {
        $this->Item->cleanup();
        $_SESSION['notice'] = 'Xóa mặt hàng cũ thành công';
    ...
    public function action_import_data()
    {
        ...
        $override_db = $this->input->post('override_db');
        $this->check_action_permission('add_update');
        $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        ...
        $location_items = $this->Location->list_item();
        ...
        foreach ($rows as $index => $row) {
            ...
            $extend_data = $extend_rows = $quantity_arr = array();
                    ...
                    if (count($field_parts) == 2) {
                        switch ($field_parts[0]) {
                            case 'basic':
                                ...
                                break;
                            case 'quantity':
                                $quantity_arr[$field_parts[1]] = $row[$excel_column];
                                break;
                            case 'extend':
                                $extend_data = array(
            ...
            if(!empty($data['measure'])) {
                ...
                $measure_items = $this->Measure->item_select_box();

                $lower_measure = mb_strtolower(trim($data['measure']), 'UTF-8');

                $measure = -1;
                if(!empty($measure_items)) {
                    foreach($measure_items as $val) {
                        $measure_tmp = mb_strtolower(trim($val['name']), 'UTF-8');
                        if($lower_measure == $measure_tmp)
                            $measure = $val['id'];
                    }

                }

                $data['measure_id'] = $measure;
                unset($data['measure']);
            }
            ...
            try {
                ...
                $update_row = false;
                $error_message = '';
                $location_item_data = array();
                $inventory_data = array();
                # $override_db Có cho phép ghi đè hay không
                if (isset($check_duplicate_field_type) && isset($check_duplicate_field_name) && !$override_db) {
                    switch ($check_duplicate_field_type) {
                        case 'basic':
                            if (!empty($data[$check_duplicate_field_name])) {
                                ...
                                $error_message = 'Có lỗi dữ liệu bị trùng lặp';
                            }
                            break;
                        case 'extend':
                ...
                if (!$exists_row) {
                        $tag_names = '';
                        # Nếu cho chọn chế độ cho phép ghi đè
                        if($override_db) {
                            # Lấy ra dữ liệu cũ
                            $du_lieu_cu = $this->Item->get_item_by_field($check_duplicate_field_name, $data[$check_duplicate_field_name]);
                            if(!empty($du_lieu_cu)) {
                                $update_row = true;
                                # Lấy ra id cũ để cập nhật các bảng liên quan
                                $item_id = $du_lieu_cu['item_id'];
                            }

                        } else $update_row = fALSE;
                        ...
                        if (empty($data['product_id'])) {
                            $data['product_id'] = 'SP-'.$index.'-';
                        }
                        if(!empty($data['category'])) {
                            $this->load->model('Category');
                            $category = $this->Category->getCategoryByName($data['category']);
                            $data['category_id'] = $category['id'];
                            unset($data['category']);
                        }
                        if(!empty($data['suppliers'])) {
                            $this->load->model('Supplier');
                            $supplier_id = $this->Supplier->getSupplierByName($data['suppliers']);
                            $data['supplier_id'] = $supplier_id['person_id'];
                        ...
                        if(empty($data['xoay_kho'])|| !is_numeric($data['xoay_kho'])) {
                            $data['xoay_kho'] = 0;
                        }    
                        if(empty($data['stop_producing'])|| !is_numeric($data['stop_producing'])) {
                            $data['stop_producing'] = 0;
                        }                            

                        # Lưu dữ liệu
                        if($update_row) {
                            $this->Item->update_item($check_duplicate_field_name, $data[$check_duplicate_field_name], $data);
                        } else {
                            $item_id = $this->Item->save($data);
                            if (!empty($tag_names)) {
                                $this->load->model('Tag');
                                $this->Tag->save_tags_for_item($item_id,$tag_names);
                            }
                        }
 

                        if(!isset($item_id)) {
                            echo json_encode(array('success'=>fALSE, 'msg'=>'Có lỗi khi import dữ liệu, vui lòng nâng cấp hệ thông hoặc liên hệ với support Lifetek để được hỗ trợ'));
                            return;
                        }

                        foreach($location_items as $location) {
                            # Dữ liệu số lượng tồn kho từng địa điểm
                            if(!empty($quantity_arr)) {
                                $location_ids = array_keys($quantity_arr);
                                if(in_array($location['location_id'], $location_ids))
                                    $so_luong_moi_cap_nhat = $quantity_arr[$location['location_id']];
                                else $so_luong_moi_cap_nhat = 0;
                            } else $so_luong_moi_cap_nhat = 0;
                            ...
                            $location_item_data[] = array(
                                ...
                                'start_date' => NULL,
                                'end_date' => NULL,
                                'quantity' => $so_luong_moi_cap_nhat,
                                'reorder_level' => NULL,
                            ...
                            (
                                ...
                                'trans_items' => $item_id,
                                'trans_user' => $employee_id,
                                'trans_comment' => 'Thêm mới sản phẩm bằng excel',
                                'trans_inventory' => $so_luong_moi_cap_nhat,
                                'location_id' => $location['location_id'],
                                'bat_dau' =>1,
                            );
                        }

                        # Nếu cập nhật ghi đè thì xóa tất cả dữ liệu cũ
                        if($update_row) {
                            $this->db->where('item_id',$item_id);
                            $this->db->delete('location_items');

                            $this->db->where('item_id',$item_id);
                            $this->db->delete('inventory',$item_id);
                        } 
                        $this->db->insert_batch('location_items', $location_item_data);
                        # Thêm mới dữ liệu cho báo cáo xuất nhập tồn
                        $this->db->insert_batch('inventory', $inventory_data);
                    ...
                    if($update_row == true && $override_db)
                        $success_rows[] = $index;
                    else{
                        $error_rows[] = $index;
                    }
        ...
        } # Kết thúc vòng lặp  foreach ($rows as $index => $row)

        echo json_encode(array('error_rows'=>$error_rows, 'success_rows'=>$success_rows, 'error_message'=> $error_message,'success' => TRUE));
    }
}
