...
class Attribute_sets extends Secure_area implements Idata_controller
{
    ...
    */
    ...
    function viewAttributes($item_id = -1)
    {
        $data = $this->_get_data($this->input->post('attribute_set_id'));
        echo $this->load->view("items/listAttribute", $data, true);
    }
    /*
    ...
    */
    function save($attribute_set_id = false)
    {
        ...
        if ($data['attribute_set_id'] = $this->Attribute_set->save($data, $attribute_set_id)) {
            ...
            $attribute_formula = $this->input->post('attribute_formula');
            $data_update_attr_values    = [];
            $data_update_attr_combine   = [];
                ...
                if (!empty($attribute_group_id)) {
                    foreach ($attribute_ids as $attribute_id) {
                        $data_update_attr_combine[] = [
                            ...
                            'formula' => $attribute_formula[$attribute_group_id][$attribute_id]
                        ];
                        
                        if ($attribute_formula[$attribute_group_id][$attribute_id]{0} == '=') {
                            $data_update_attr_values [] = [
                                'attribute_id' => $attribute_id,
                                'entity_value' => $attribute_formula[$attribute_group_id][$attribute_id]
                            ];
                        }
                    }
                }
            }
            $this->Attribute->update_batch_attribute_values($data_update_attr_values);
            $this->Attribute_set->combine($data_update_attr_combine);
        } else {
            /* Failure */
    ...
    */
    ...
    public function testFormula()
    {
        ...
        if($post['strCode']) {
            ...
            $strCode = $post['strCode'];
             //create new ConvertFormula object
            $atributeCodes = new ConvertFormula($strCode);
            
            //get list of variable from input string
            $listVariable = $atributeCodes->getListVariable();
            
            $data['listVariable'] = $listVariable;
            $data['formula']      = $strCode;
            echo json_encode(array('html'=>$this->load->view('attribute_sets/test_formula', $data, true)));
        }
        else {
            
            if (!empty($post['input_test'])) {
                $result     = new ConvertFormula($post['strCodeTestFormula'],$post['input_test']);
                $_result    = $result->executeFormula();
