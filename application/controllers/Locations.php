...
class Locations extends Secure_area implements Idata_controller
{
	...
	function index($offset=0)
	{
		...
		$data['locations'] = $this->Location->getLocationsWithChild();
...
// 		    return ['id' => $location['location_id'], 'name' => $location['name']];
// 		},$locations);
// 		$data = array();
// 		array_unshift($locations, ['id' => 0, 'name' => 'Chọn kho cha ...']);
		
		$data['locations'] = $locations = $this->Location->getLocationsWithChild();
        ...
        $data['location_id'] = $location_id = $this->uri->segment(3);

        if($location_id != -1) {
            $location_group_list = $this->Location->get_location_group_employees($location_id);
            $data['location_group_list'] = $location_group_list;
        }
        $data['types'] = $this->Location->getTypes();
    ...
    function location_save() {
        ...
        $post['address'] = filter_trim_space($post['address']);
        $post['phone'] = filter_trim_space($post['phone']);
        ...
        if(!isset($post['employees'])) {
            $flagError = true;
            $errors['employees'] = 'Mỗi địa điểm phải chọn ít nhất một nhân viên.';
        }
        ...
        if($flagError == true) {
            ...
            $location_data = array(
                ...
                'type'=> $this->input->post('location_type') ? $this->input->post('location_type') : 'kho',
                'parent_id'=> $this->input->post('location_parent') ? $this->input->post('location_parent') : '0',
                'color' => $this->input->post('color') ? $this->input->post('color') : NULL,
                'address'=>$this->input->post('address'),
                'code'=>$this->input->post('code'),
                'phone'=>$this->input->post('phone'),
                'fax'=>$this->input->post('fax'),
                'sale_office'=>$this->input->post('sale_office'),
                ...
                'type'=>'cong_ty',
                'hosted_checkout_merchant_password'=>$this->input->request('hosted_checkout_merchant_password'),//Use REQUEST to avoid url encoding 
            );

            
            if($location_id==-1){
            	$this->db->insert('phppos_locations', $location_data);
            	$location_id = $this->db->insert_id();
            }
            else{
            	$this->db->where('location_id', $location_id);
            	$this->db->update('phppos_locations', $location_data);
            }
            $this->Location->assign_employees_to_location($location_id,$employees);
        }
...
//		}
	...
	function delete()
	{
		...
		// TODO
		$locations_to_delete = $this->Location->getLocationsWithChild($ids);
		$locations_to_delete = array_map(function($location){
    ...
    function location_group() {
        ...
        $data['slb_group'] = $this->Group->item_select_box();
        $this->load->view('locations/form_location_group', $data);
    }
    ...
    function add_group_section() {
        ...
        if(!empty($post)) {
            $data = $arrParams;
            $group_info = $this->Group->get_item(array('id'=>$arrParams['location_group_id']));
            if(empty($group_info)) return;

            $data['group_info'] = $group_info;
            $this->load->view('locations/partials/location_group', $data);
        }
    }
}
?>