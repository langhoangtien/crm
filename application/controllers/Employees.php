...
class Employees extends Person_controller
{
    ...
    */
    ...
    function _get_employee_data($employee_id)
    {
        ...
        $groups = $this->Group->get_all_group()->result();
    ...
    */
    ...
    function view($employee_id = -1, $redirect_code = 0)
    {
        ...
        $data['levels'] = $this->Employee->get_level_by_rank();
        $data['rank_id']= $this->Employee->get_rank_by_level($data['person_info']->level_id);
        $data['redirect_code'] = $redirect_code;
        if($employee_id != -1) {
            $data['commission_info'] = $this->Employee->get_location_group_commission($employee_id);
        }
        $data['employee_id'] = $employee_id;
                if($this->input->post('group_id'))
                {
                    $data['group_id'] = $this->input->post('group_id');
                    echo json_encode($this->load->view('employees/form_permission_list',$data,true));
                }
                else
                {
    ...
    */
    ...
    {   
        ...
        $this->check_action_permission('add_update_rank');
        $this->load->helper('url');
        $data['ranks'] = $this->Employee->get_ranks();
        $id = $data['ranks'][0]['id'] ? $data['ranks'][0]['id'] :1;
        $data['level'] = $this->Employee->get_level_by_rank();
        $data['level2'] = $this->Employee->get_level_by_rank($id);
        $this->load->view('employees/ranks',$data);
    ...
    public function get_level()
    {
        ...
        $data['level'] = $this->Employee->get_level_by_rank($id);
        $this->load->view('employees/employees-level',$data);
        }
        if($this->input->post('rank_id'))
        {
        $id = $this->input->post('rank_id') ? $this->input->post('rank_id') :1;
        $data['rank'] = $this->Employee->get_level_by_rank($id);
        $this->load->view('employees/employees-level',$data); 
        }
    ...
    public function level_save(){
        ...
        {   
...

            {
                ...
                {
                    $dt[$key]['name'] = $value;
                    $dt[$key]['salary'] = str_replace(',', '',(string)$data['level_salary'][$key]);
                    $dt[$key]['id'] = $data['level_id'][$key];
                    $dt[$key]['parent_id'] = $data['parent_id'];
                }
                if ($this->Employee->level_save($data['parent_id'],$dt)) 

                {
                    $_SESSION['notice'] = " Lưu thành công";
                    redirect(base_url('employees/ranks'),'refresh');
                }

            }
            else
            {
                if($this->Employee->level_save($data['parent_id']))
                {
                    $_SESSION['notice'] = " Lưu thành công";
                    redirect(base_url('employees/ranks'),'refresh');
                }
